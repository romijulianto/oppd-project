...
public partial class OverridePS {
    ...
    /// mtReasonList
    /// </summary>
    public static MtReasonList mtReasonList
    {
        get => HttpData.Get<MtReasonList>("mtReasonList")!;
        set => HttpData["mtReasonList"] = value;
    }

    /// <summary>
    /// Page class for mt_reason
    /// </summary>
    public class MtReasonList : MtReasonListBase
    {
        // Constructor
        public MtReasonList(Controller controller) : base(controller)
        {
        }

        // Constructor
        public MtReasonList() : base()
        {
        }
    }
    ...
    /// Page base class
    /// </summary>
    public class MtReasonListBase : MtReason
    {
        // Page ID
        ...
        // Table name
        public string TableName { get; set; } = "mt_reason";

        // Page object name
        public string PageObjName = "mtReasonList";

        // Title
        public string? Title = null; // Title for <title> tag

        // Grid form hidden field names
        public string FormName = "fmt_reasonlist";
        ...
        public MtReasonListBase()
        {
            ...
            // Table object (mtReason)
            if (mtReason == null || mtReason is MtReason)
                mtReason = this;
            ...
            MultiDeleteUrl = "MtReasonDelete";
            MultiUpdateUrl = "MtReasonUpdate";
        ...
        // Page name
        public string PageName => "MtReasonList";
        ...
        {
            id.SetVisibility();
            Reason_Type.SetVisibility();
            Reason_Description.SetVisibility();
        }

        // Constructor
        public MtReasonListBase(Controller? controller = null): this() { // DN
            if (controller != null)
                ...
                if (Response != null && !Response.HasStarted) {
                    ...
                    if (IsModal) { // Show as modal
                        ...
                        if (pageName != ListUrl) { // Not List page
                            ...
                            result.Add("view", pageName == "MtReasonView" ? "1" : "0"); // If View page, no primary button
                        } else { // List page
        ...
        public async Task<string> GetFilterList()
        {
            ...
            filters.Merge(JObject.Parse(Reason_Type.AdvancedSearch.ToJson())); // Field Reason_Type
            filters.Merge(JObject.Parse(Reason_Description.AdvancedSearch.ToJson())); // Field Reason_Description
        ...
        protected bool RestoreFilterList() {
            ...
            // Field Reason_Type
            if (filter?.TryGetValue("x_Reason_Type", out sv) ?? false) {
                Reason_Type.AdvancedSearch.SearchValue = sv;
                Reason_Type.AdvancedSearch.SearchOperator = filter["z_Reason_Type"];
                Reason_Type.AdvancedSearch.SearchCondition = filter["v_Reason_Type"];
                Reason_Type.AdvancedSearch.SearchValue2 = filter["y_Reason_Type"];
                Reason_Type.AdvancedSearch.SearchOperator2 = filter["w_Reason_Type"];
                Reason_Type.AdvancedSearch.Save();
            }

            // Field Reason_Description
            if (filter?.TryGetValue("x_Reason_Description", out sv) ?? false) {
                Reason_Description.AdvancedSearch.SearchValue = sv;
                Reason_Description.AdvancedSearch.SearchOperator = filter["z_Reason_Description"];
                Reason_Description.AdvancedSearch.SearchCondition = filter["v_Reason_Description"];
                Reason_Description.AdvancedSearch.SearchValue2 = filter["y_Reason_Description"];
                Reason_Description.AdvancedSearch.SearchOperator2 = filter["w_Reason_Description"];
                Reason_Description.AdvancedSearch.Save();
            }
        ...
        public string BasicSearchWhere(bool def = false) {
            ...
            searchFlds.Add(Reason_Type);
            searchFlds.Add(Reason_Description);
        ...
        protected void SetupSortOrder() {
            ...
            if (Get("order", out StringValues sv)) {
                ...
                UpdateSort(id); // id
                UpdateSort(Reason_Type); // Reason_Type
                UpdateSort(Reason_Description); // Reason_Description
        ...
        public async Task RenderListOptions()
        {
            ...
            if (isVisible) {
                if (ModalView && !IsMobile())
                    listOption?.SetBody($@"<a class=""ew-row-link ew-view"" title=""{viewcaption}"" data-table=""mt_reason"" data-caption=""{viewcaption}"" data-ew-action=""modal"" data-action=""view"" data-ajax=""" + (UseAjaxActions ? "true" : "false") + "\" data-url=\"" + HtmlEncode(AppPath(ViewUrl)) + "\" data-btn=\"null\">" + Language.Phrase("ViewLink") + "</a>");
                else
            ...
            if (isVisible) {
                if (ModalEdit && !IsMobile())
                    listOption?.SetBody($@"<a class=""ew-row-link ew-edit"" title=""{editcaption}"" data-table=""mt_reason"" data-caption=""{editcaption}"" data-ew-action=""modal"" data-action=""edit"" data-ajax=""" + (UseAjaxActions ? "true" : "false") + "\" data-url=\"" + HtmlEncode(AppPath(EditUrl)) + "\" data-btn=\"SaveBtn\">" + Language.Phrase("EditLink") + "</a>");
                else
            ...
            if (isVisible) {
                if (ModalAdd && !IsMobile())
                    listOption?.SetBody($@"<a class=""ew-row-link ew-copy"" title=""{copycaption}"" data-table=""mt_reason"" data-caption=""{copycaption}"" data-ew-action=""modal"" data-action=""add"" data-ajax=""" + (UseAjaxActions ? "true" : "false") + "\" data-url=\"" + HtmlEncode(AppPath(CopyUrl)) + "\" data-btn=\"AddBtn\">" + Language.Phrase("CopyLink") + "</a>");
                else
            ...
            if (listOption != null && !IsExport() && CurrentAction == "") {
                ...
                foreach (var (key, act) in ListActions.Items) {
                    if (act.Select == Config.ActionSingle && act.Allowed) {
                        ...
                        string link = "<li><button type=\"button\" class=\"dropdown-item ew-action ew-list-action\" data-caption=\"" + HtmlTitle(caption) + "\" data-ew-action=\"submit\" form=\"fmt_reasonlist\" data-key=\"" + KeyToJson(true) + "\"" + act.ToDataAttrs() + ">" + icon + " " + caption + "</button></li>";
                        if (!Empty(link)) {
                            links.Add(link);
                            if (Empty(body)) // Setup first button
                                body = "<button type=\"button\" class=\"btn btn-default ew-action ew-list-action\" title=\"" + HtmlTitle(caption) + "\" data-caption=\"" + HtmlTitle(caption) + "\" data-ew-action=\"submit\" form=\"fmt_reasonlist\" data-key=\"" + KeyToJson(true) + "\"" + act.ToDataAttrs() + ">" + icon + caption + "</button>";
                        }
                    }
                }
        ...
        protected void SetupOtherOptions() {
            ...
            if (ModalAdd && !IsMobile())
                item.Body = $@"<a class=""ew-add-edit ew-add"" title=""{addTitle}"" data-table=""mt_reason"" data-caption=""{addTitle}"" data-ew-action=""modal"" data-action=""add"" data-ajax=""" + (UseAjaxActions ? "true" : "false") + "\" data-url=\"" + HtmlEncode(AppPath(AddUrl)) + "\" data-btn=\"AddBtn\">" + Language.Phrase("AddLink") + "</a>";
            else
            ...
            item.Body = $@"<button type=""button"" class=""ew-action ew-multi-delete"" title=""{deleteTitle}"" data-caption=""{deleteTitle}"" form=""fmt_reasonlist""" +
            ...
            if (UseColumnVisibility) {
                ...
                CreateColumnOption(option.Add("Reason_Type")); // DN
                CreateColumnOption(option.Add("Reason_Description")); // DN
            }
            ...
            item.Body = "<a class=\"ew-save-filter\" data-form=\"fmt_reasonsrch\" data-ew-action=\"none\">" + Language.Phrase("SaveCurrentFilter") + "</a>";
            item.Visible = true;
            item = FilterOptions.Add("deletefilter");
            item.Body = "<a class=\"ew-delete-filter\" data-form=\"fmt_reasonsrch\" data-ew-action=\"none\">" + Language.Phrase("DeleteFilter") + "</a>";
            item.Visible = true;
        ...
        public void RenderOtherOptions()
        {
            ...
            var options = OtherOptions;
                ...
                foreach (var (key, act) in ListActions.Items.Where(kvp => kvp.Value.Select == Config.ActionMultiple)) {
                    ...
                    item.Body = "<<button type=\"button\" class=\"btn btn-default ew-action ew-list-action\" title=\"" + HtmlEncode(caption) + "\" data-caption=\"" + HtmlEncode(caption) + "\" data-ew-action=\"submit\" form=\"fmt_reasonlist\"" + act.ToDataAttrs() + ">" + icon + "</button>";
        ...
        public async Task SetupRow()
        {
            ...
            RowAttrs.Add("data-rowindex", ConvertToString(mtReasonList.RowCount));
            RowAttrs.Add("data-key", GetKey(true));
            RowAttrs.Add("id", "r" + ConvertToString(mtReasonList.RowCount) + "_mt_reason");
            RowAttrs.Add("data-rowtype", ConvertToString((int)RowType));
            RowAttrs.AppendClass(mtReasonList.RowCount % 2 != 1 ? "ew-table-alt-row" : "");
            if (IsAdd && mtReasonList.RowType == RowType.Add || IsEdit && mtReasonList.RowType == RowType.Edit) // Inline-Add/Edit row
        ...
        public async Task LoadRowValues(Dictionary<string, object>? row)
        {
            ...
            Reason_Type.SetDbValue(row["Reason_Type"]);
            Reason_Description.SetDbValue(row["Reason_Description"]);
        }
        ...
        protected Dictionary<string, object> NewRow() {
            ...
            row.Add("Reason_Type", Reason_Type.DefaultValue ?? DbNullValue); // DN
            row.Add("Reason_Description", Reason_Description.DefaultValue ?? DbNullValue); // DN
            return row;
        }
        ...
        public async Task RenderRow()
        {
            ...
            if (RowType == RowType.View) {
                ...
                // Reason_Type
                Reason_Type.ViewValue = ConvertToString(Reason_Type.CurrentValue); // DN
                Reason_Type.ViewCustomAttributes = "";

                // Reason_Description
                Reason_Description.ViewValue = ConvertToString(Reason_Description.CurrentValue); // DN
                Reason_Description.ViewCustomAttributes = "";
                ...
                // Reason_Type
                Reason_Type.HrefValue = "";
                Reason_Type.TooltipValue = "";

                // Reason_Description
                Reason_Description.HrefValue = "";
                Reason_Description.TooltipValue = "";
            }
        ...
        protected void SetupSearchOptions() {
            ...
            item.Body = "<a class=\"btn btn-default ew-search-toggle" + searchToggleClass + "\" role=\"button\" title=\"" + Language.Phrase("SearchPanel") + "\" data-caption=\"" + Language.Phrase("SearchPanel") + "\" data-ew-action=\"search-toggle\" data-form=\"fmt_reasonsrch\" aria-pressed=\"" + (searchToggleClass == " active" ? "true" : "false") + "\">" + Language.Phrase("SearchLink") + "</a>";
            item.Visible = true;
