...
public partial class OverridePS {
    ...
    /// </summary>
    [MaybeNull]
    public static MtReason mtReason
    {
        get => HttpData.Resolve<MtReason>("mt_reason");
        set => HttpData["mt_reason"] = value;
    }

    /// <summary>
    /// Table class for mt_reason
    /// </summary>
    public class MtReason : DbTable, IQueryFactory
    {
        ...
        public readonly DbField<SqlDbType> Reason_Type;

        public readonly DbField<SqlDbType> Reason_Description;
        ...
        public MtReason()
        {
            ...
            Name = "mt_reason";
            Type = "TABLE";
            UpdateTable = "dbo.mt_reason"; // Update Table
            DbId = "DB"; // DN
            ExportAll = true;
            ...
            id = new (this, "x_id", 3, SqlDbType.Int) {
                ...
                CustomMessage = Language.FieldPhrase("mt_reason", "id", "CustomMsg"),
                IsUpload = false
            };
            Fields.Add("id", id);

            // Reason_Type
            Reason_Type = new (this, "x_Reason_Type", 200, SqlDbType.VarChar) {
                Name = "Reason_Type",
                Expression = "[Reason_Type]",
                UseBasicSearch = true,
                BasicSearchExpression = "[Reason_Type]",
                DateTimeFormat = -1,
                VirtualExpression = "[Reason_Type]",
                IsVirtual = false,
                ForceSelection = false,
                SelectMultiple = false,
                VirtualSearch = false,
                ...
                CustomMessage = Language.FieldPhrase("mt_reason", "Reason_Type", "CustomMsg"),
                IsUpload = false
            };
            Fields.Add("Reason_Type", Reason_Type);

            // Reason_Description
            Reason_Description = new (this, "x_Reason_Description", 200, SqlDbType.VarChar) {
                Name = "Reason_Description",
                Expression = "[Reason_Description]",
                UseBasicSearch = true,
                BasicSearchExpression = "[Reason_Description]",
                DateTimeFormat = -1,
                VirtualExpression = "[Reason_Description]",
                IsVirtual = false,
                ForceSelection = false,
                SelectMultiple = false,
                VirtualSearch = false,
                ...
                InputTextType = "text",
                SearchOperators = new () { "=", "<>", "IN", "NOT IN", "STARTS WITH", "NOT STARTS WITH", "LIKE", "NOT LIKE", "ENDS WITH", "NOT ENDS WITH", "IS EMPTY", "IS NOT EMPTY", "IS NULL", "IS NOT NULL" },
                CustomMessage = Language.FieldPhrase("mt_reason", "Reason_Description", "CustomMsg"),
                IsUpload = false
            };
            Fields.Add("Reason_Description", Reason_Description);
        ...
        public void LoadDbValues(Dictionary<string, object>? row)
        {
            ...
            try {
                ...
                Reason_Type.DbValue = row["Reason_Type"]; // Set DB value only
                Reason_Description.DbValue = row["Reason_Description"]; // Set DB value only
            } catch {}
        }
        ...
        {
            if (SameString(pageName, "MtReasonView"))
                return Language.Phrase("View");
            else if (SameString(pageName, "MtReasonEdit"))
                return Language.Phrase("Edit");
            else if (SameString(pageName, "MtReasonAdd"))
        ...
        public string GetApiPageName(string action)
        {
            return action.ToLowerInvariant() switch {
                Config.ApiViewAction => "MtReasonView",
                Config.ApiAddAction => "MtReasonAdd",
                Config.ApiEditAction => "MtReasonEdit",
                Config.ApiDeleteAction => "MtReasonDelete",
                Config.ApiListAction => "MtReasonList",
                _ => String.Empty
            };
        }
        ...
        // List URL
        public string ListUrl => "MtReasonList";
        ...
        public string GetViewUrl(string parm = "")
        {
            string url = "";
            if (!Empty(parm))
                url = KeyUrl("MtReasonView", parm);
            else
                url = KeyUrl("MtReasonView", Config.TableShowDetail + "=");
            return AddMasterUrl(url);
        }

        // Add URL
        public string AddUrl { get; set; } = "MtReasonAdd";
        ...
        {
            string url = "";
            url = KeyUrl("MtReasonEdit", parm);
            return AppPath(AddMasterUrl(url)); // DN
        }

        // Inline edit URL
        public string InlineEditUrl =>
            AppPath(AddMasterUrl(KeyUrl("MtReasonList", "action=edit"))); // DN
        ...
        {
            string url = "";
            url = KeyUrl("MtReasonAdd", parm);
            return AppPath(AddMasterUrl(url)); // DN
        }

        // Inline copy URL
        public string InlineCopyUrl =>
            AppPath(AddMasterUrl(KeyUrl("MtReasonList", "action=copy"))); // DN

        // Delete URL
        public string DeleteUrl => UseAjaxActions && Param<bool>("infinitescroll") && CurrentPageID() == "list"
            ? AppPath(KeyUrl(Config.ApiUrl + Config.ApiDeleteAction + "/" + TableVar))
            : AppPath(KeyUrl("MtReasonDelete")); // DN
        ...
        public void LoadListRowValues(DbDataReader? dr)
        {
            if (dr == null)
                return;
            id.SetDbValue(dr["id"]);
            Reason_Type.SetDbValue(dr["Reason_Type"]);
            Reason_Description.SetDbValue(dr["Reason_Description"]);
        }

        // Render list content
        public async Task<string> RenderListContent(string filter)
        {
            string pageName = "MtReasonList";
            ...
            if (!doc.ExportCustom) {
                ...
                if (doc.Horizontal) { // Horizontal format, write header
                    ...
                    if (exportType == "view") {
                        ...
                        doc.ExportCaption(Reason_Type);
                        doc.ExportCaption(Reason_Description);
                    } else {
                        doc.ExportCaption(id);
                        doc.ExportCaption(Reason_Type);
                        doc.ExportCaption(Reason_Description);
                ...
                if (recCnt >= startRec) {
                    ...
                    if (!doc.ExportCustom) {
                        ...
                        if (exportType == "view") {
                            ...
                            await doc.ExportField(Reason_Type);
                            await doc.ExportField(Reason_Description);
                        } else {
                            await doc.ExportField(id);
                            await doc.ExportField(Reason_Type);
                            await doc.ExportField(Reason_Description);
