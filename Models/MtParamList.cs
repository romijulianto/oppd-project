...
public partial class OverridePS {
    ...
    /// mtParamList
    /// </summary>
    public static MtParamList mtParamList
    {
        get => HttpData.Get<MtParamList>("mtParamList")!;
        set => HttpData["mtParamList"] = value;
    }

    /// <summary>
    /// Page class for mt_param
    /// </summary>
    public class MtParamList : MtParamListBase
    {
        // Constructor
        public MtParamList(Controller controller) : base(controller)
        {
        }

        // Constructor
        public MtParamList() : base()
        {
        }
    }
    ...
    /// Page base class
    /// </summary>
    public class MtParamListBase : MtParam
    {
        // Page ID
        ...
        // Page object name
        public string PageObjName = "mtParamList";

        // Title
        public string? Title = null; // Title for <title> tag

        // Grid form hidden field names
        public string FormName = "fmt_paramlist";
        ...
        public MtParamListBase()
        {
            ...
            MultiDeleteUrl = "MtParamDelete";
            MultiUpdateUrl = "MtParamUpdate";
        ...
        // Page name
        public string PageName => "MtParamList";
        ...
        // Constructor
        public MtParamListBase(Controller? controller = null): this() { // DN
            if (controller != null)
        ...
        public async Task<string> GetFilterList()
        {
            ...
            filters.Merge(JObject.Parse(Param_Type.AdvancedSearch.ToJson())); // Field Param_Type
            filters.Merge(JObject.Parse(Param_Name.AdvancedSearch.ToJson())); // Field Param_Name
            filters.Merge(JObject.Parse(Param_Value.AdvancedSearch.ToJson())); // Field Param_Value
            filters.Merge(JObject.Parse(Param_Status.AdvancedSearch.ToJson())); // Field Param_Status
        ...
        protected bool RestoreFilterList() {
            ...
            // Field Param_Type
            if (filter?.TryGetValue("x_Param_Type", out sv) ?? false) {
                Param_Type.AdvancedSearch.SearchValue = sv;
                Param_Type.AdvancedSearch.SearchOperator = filter["z_Param_Type"];
                Param_Type.AdvancedSearch.SearchCondition = filter["v_Param_Type"];
                Param_Type.AdvancedSearch.SearchValue2 = filter["y_Param_Type"];
                Param_Type.AdvancedSearch.SearchOperator2 = filter["w_Param_Type"];
                Param_Type.AdvancedSearch.Save();
            }

            // Field Param_Name
            if (filter?.TryGetValue("x_Param_Name", out sv) ?? false) {
                Param_Name.AdvancedSearch.SearchValue = sv;
                Param_Name.AdvancedSearch.SearchOperator = filter["z_Param_Name"];
                Param_Name.AdvancedSearch.SearchCondition = filter["v_Param_Name"];
                Param_Name.AdvancedSearch.SearchValue2 = filter["y_Param_Name"];
                Param_Name.AdvancedSearch.SearchOperator2 = filter["w_Param_Name"];
                Param_Name.AdvancedSearch.Save();
            }

            // Field Param_Value
            if (filter?.TryGetValue("x_Param_Value", out sv) ?? false) {
                Param_Value.AdvancedSearch.SearchValue = sv;
                Param_Value.AdvancedSearch.SearchOperator = filter["z_Param_Value"];
                Param_Value.AdvancedSearch.SearchCondition = filter["v_Param_Value"];
                Param_Value.AdvancedSearch.SearchValue2 = filter["y_Param_Value"];
                Param_Value.AdvancedSearch.SearchOperator2 = filter["w_Param_Value"];
                Param_Value.AdvancedSearch.Save();
            }

            // Field Param_Status
            if (filter?.TryGetValue("x_Param_Status", out sv) ?? false) {
                Param_Status.AdvancedSearch.SearchValue = sv;
                Param_Status.AdvancedSearch.SearchOperator = filter["z_Param_Status"];
                Param_Status.AdvancedSearch.SearchCondition = filter["v_Param_Status"];
                Param_Status.AdvancedSearch.SearchValue2 = filter["y_Param_Status"];
                Param_Status.AdvancedSearch.SearchOperator2 = filter["w_Param_Status"];
                Param_Status.AdvancedSearch.Save();
            }
        ...
        public string BasicSearchWhere(bool def = false) {
            ...
            searchFlds.Add(Param_Type);
            searchFlds.Add(Param_Name);
            searchFlds.Add(Param_Value);
            searchFlds.Add(Param_Status);
        ...
        protected void SetupSortOrder() {
            ...
            if (Get("order", out StringValues sv)) {
                ...
                UpdateSort(id); // id
                UpdateSort(Param_Type); // Param_Type
                UpdateSort(Param_Name); // Param_Name
                UpdateSort(Param_Value); // Param_Value
                UpdateSort(Param_Status); // Param_Status
        ...
        public async Task RenderListOptions()
        {
            ...
            if (isVisible) {
                if (ModalView && !IsMobile())
                    listOption?.SetBody($@"<a class=""ew-row-link ew-view"" title=""{viewcaption}"" data-table=""mt_param"" data-caption=""{viewcaption}"" data-ew-action=""modal"" data-action=""view"" data-ajax=""" + (UseAjaxActions ? "true" : "false") + "\" data-url=\"" + HtmlEncode(AppPath(ViewUrl)) + "\" data-btn=\"null\">" + Language.Phrase("ViewLink") + "</a>");
                else
            ...
            if (isVisible) {
                if (ModalEdit && !IsMobile())
                    listOption?.SetBody($@"<a class=""ew-row-link ew-edit"" title=""{editcaption}"" data-table=""mt_param"" data-caption=""{editcaption}"" data-ew-action=""modal"" data-action=""edit"" data-ajax=""" + (UseAjaxActions ? "true" : "false") + "\" data-url=\"" + HtmlEncode(AppPath(EditUrl)) + "\" data-btn=\"SaveBtn\">" + Language.Phrase("EditLink") + "</a>");
                else
            ...
            if (isVisible) {
                if (ModalAdd && !IsMobile())
                    listOption?.SetBody($@"<a class=""ew-row-link ew-copy"" title=""{copycaption}"" data-table=""mt_param"" data-caption=""{copycaption}"" data-ew-action=""modal"" data-action=""add"" data-ajax=""" + (UseAjaxActions ? "true" : "false") + "\" data-url=\"" + HtmlEncode(AppPath(CopyUrl)) + "\" data-btn=\"AddBtn\">" + Language.Phrase("CopyLink") + "</a>");
                else
            ...
            if (listOption != null && !IsExport() && CurrentAction == "") {
                ...
                foreach (var (key, act) in ListActions.Items) {
                    if (act.Select == Config.ActionSingle && act.Allowed) {
                        ...
                        string link = "<li><button type=\"button\" class=\"dropdown-item ew-action ew-list-action\" data-caption=\"" + HtmlTitle(caption) + "\" data-ew-action=\"submit\" form=\"fmt_paramlist\" data-key=\"" + KeyToJson(true) + "\"" + act.ToDataAttrs() + ">" + icon + " " + caption + "</button></li>";
                        if (!Empty(link)) {
                            links.Add(link);
                            if (Empty(body)) // Setup first button
                                body = "<button type=\"button\" class=\"btn btn-default ew-action ew-list-action\" title=\"" + HtmlTitle(caption) + "\" data-caption=\"" + HtmlTitle(caption) + "\" data-ew-action=\"submit\" form=\"fmt_paramlist\" data-key=\"" + KeyToJson(true) + "\"" + act.ToDataAttrs() + ">" + icon + caption + "</button>";
                        }
                    }
                }
        ...
        protected void SetupOtherOptions() {
            ...
            if (ModalAdd && !IsMobile())
                item.Body = $@"<a class=""ew-add-edit ew-add"" title=""{addTitle}"" data-table=""mt_param"" data-caption=""{addTitle}"" data-ew-action=""modal"" data-action=""add"" data-ajax=""" + (UseAjaxActions ? "true" : "false") + "\" data-url=\"" + HtmlEncode(AppPath(AddUrl)) + "\" data-btn=\"AddBtn\">" + Language.Phrase("AddLink") + "</a>";
            else
            ...
            item.Body = $@"<button type=""button"" class=""ew-action ew-multi-delete"" title=""{deleteTitle}"" data-caption=""{deleteTitle}"" form=""fmt_paramlist""" +
            ...
            if (UseColumnVisibility) {
                ...
                CreateColumnOption(option.Add("Param_Type")); // DN
                CreateColumnOption(option.Add("Param_Name")); // DN
                CreateColumnOption(option.Add("Param_Value")); // DN
                CreateColumnOption(option.Add("Param_Status")); // DN
            }
            ...
            item.Body = "<a class=\"ew-save-filter\" data-form=\"fmt_paramsrch\" data-ew-action=\"none\">" + Language.Phrase("SaveCurrentFilter") + "</a>";
            item.Visible = true;
            item = FilterOptions.Add("deletefilter");
            item.Body = "<a class=\"ew-delete-filter\" data-form=\"fmt_paramsrch\" data-ew-action=\"none\">" + Language.Phrase("DeleteFilter") + "</a>";
            item.Visible = true;
        ...
        public void RenderOtherOptions()
        {
            ...
            var options = OtherOptions;
                ...
                foreach (var (key, act) in ListActions.Items.Where(kvp => kvp.Value.Select == Config.ActionMultiple)) {
                    ...
                    item.Body = "<<button type=\"button\" class=\"btn btn-default ew-action ew-list-action\" title=\"" + HtmlEncode(caption) + "\" data-caption=\"" + HtmlEncode(caption) + "\" data-ew-action=\"submit\" form=\"fmt_paramlist\"" + act.ToDataAttrs() + ">" + icon + "</button>";
        ...
        public async Task SetupRow()
        {
            ...
            RowAttrs.Add("data-rowindex", ConvertToString(mtParamList.RowCount));
            RowAttrs.Add("data-key", GetKey(true));
            RowAttrs.Add("id", "r" + ConvertToString(mtParamList.RowCount) + "_mt_param");
            RowAttrs.Add("data-rowtype", ConvertToString((int)RowType));
            RowAttrs.AppendClass(mtParamList.RowCount % 2 != 1 ? "ew-table-alt-row" : "");
            if (IsAdd && mtParamList.RowType == RowType.Add || IsEdit && mtParamList.RowType == RowType.Edit) // Inline-Add/Edit row
        ...
        public async Task RenderRow()
        {
            ...
            if (RowType == RowType.View) {
                ...
                Param_Type.TooltipValue = "";

                // Param_Name
                Param_Name.HrefValue = "";
                Param_Name.TooltipValue = "";

                // Param_Value
                Param_Value.HrefValue = "";
                Param_Value.TooltipValue = "";

                // Param_Status
                Param_Status.HrefValue = "";
                Param_Status.TooltipValue = "";
            }
        ...
        protected void SetupSearchOptions() {
            ...
            item.Body = "<a class=\"btn btn-default ew-search-toggle" + searchToggleClass + "\" role=\"button\" title=\"" + Language.Phrase("SearchPanel") + "\" data-caption=\"" + Language.Phrase("SearchPanel") + "\" data-ew-action=\"search-toggle\" data-form=\"fmt_paramsrch\" aria-pressed=\"" + (searchToggleClass == " active" ? "true" : "false") + "\">" + Language.Phrase("SearchLink") + "</a>";
            item.Visible = true;
