...
public partial class OverridePS {
    ...
    /// mtUserList
    /// </summary>
    public static MtUserList mtUserList
    {
        get => HttpData.Get<MtUserList>("mtUserList")!;
        set => HttpData["mtUserList"] = value;
    }

    /// <summary>
    /// Page class for mt_user
    /// </summary>
    public class MtUserList : MtUserListBase
    {
        // Constructor
        public MtUserList(Controller controller) : base(controller)
        {
        }

        // Constructor
        public MtUserList() : base()
        {
        }
    }
    ...
    /// Page base class
    /// </summary>
    public class MtUserListBase : MtUser
    {
        // Page ID
        ...
        // Page object name
        public string PageObjName = "mtUserList";

        // Title
        public string? Title = null; // Title for <title> tag

        // Grid form hidden field names
        public string FormName = "fmt_userlist";
        ...
        public MtUserListBase()
        {
            ...
            MultiDeleteUrl = "MtUserDelete";
            MultiUpdateUrl = "MtUserUpdate";
        ...
        // Page name
        public string PageName => "MtUserList";
        ...
        // Constructor
        public MtUserListBase(Controller? controller = null): this() { // DN
            if (controller != null)
        ...
        public async Task<string> GetFilterList()
        {
            ...
            filters.Merge(JObject.Parse(UserPassword.AdvancedSearch.ToJson())); // Field UserPassword
            filters.Merge(JObject.Parse(_UserLevel.AdvancedSearch.ToJson())); // Field UserLevel
            filters.Merge(JObject.Parse(UserEmail.AdvancedSearch.ToJson())); // Field UserEmail
            ...
            filters.Merge(JObject.Parse(Department.AdvancedSearch.ToJson())); // Field Department
        ...
        protected bool RestoreFilterList() {
            ...
            // Field UserPassword
            if (filter?.TryGetValue("x_UserPassword", out sv) ?? false) {
                UserPassword.AdvancedSearch.SearchValue = sv;
                UserPassword.AdvancedSearch.SearchOperator = filter["z_UserPassword"];
                UserPassword.AdvancedSearch.SearchCondition = filter["v_UserPassword"];
                UserPassword.AdvancedSearch.SearchValue2 = filter["y_UserPassword"];
                UserPassword.AdvancedSearch.SearchOperator2 = filter["w_UserPassword"];
                UserPassword.AdvancedSearch.Save();
            }
            ...
            // Field UserEmail
            if (filter?.TryGetValue("x_UserEmail", out sv) ?? false) {
                UserEmail.AdvancedSearch.SearchValue = sv;
                UserEmail.AdvancedSearch.SearchOperator = filter["z_UserEmail"];
                UserEmail.AdvancedSearch.SearchCondition = filter["v_UserEmail"];
                UserEmail.AdvancedSearch.SearchValue2 = filter["y_UserEmail"];
                UserEmail.AdvancedSearch.SearchOperator2 = filter["w_UserEmail"];
                UserEmail.AdvancedSearch.Save();
            }
            ...
            // Field Department
            if (filter?.TryGetValue("x_Department", out sv) ?? false) {
                Department.AdvancedSearch.SearchValue = sv;
                Department.AdvancedSearch.SearchOperator = filter["z_Department"];
                Department.AdvancedSearch.SearchCondition = filter["v_Department"];
                Department.AdvancedSearch.SearchValue2 = filter["y_Department"];
                Department.AdvancedSearch.SearchOperator2 = filter["w_Department"];
                Department.AdvancedSearch.Save();
            }
        ...
        public string BasicSearchWhere(bool def = false) {
            ...
            searchFlds.Add(UserPassword);
            searchFlds.Add(UserEmail);
            searchFlds.Add(Full_Name);
            searchFlds.Add(Position);
            searchFlds.Add(Department);
        ...
        protected void SetupSortOrder() {
            ...
            if (Get("order", out StringValues sv)) {
                ...
                UpdateSort(UserPassword); // UserPassword
                UpdateSort(_UserLevel); // UserLevel
                UpdateSort(UserEmail); // UserEmail
                UpdateSort(Full_Name); // Full_Name
                UpdateSort(Position); // Position
                UpdateSort(Department); // Department
        ...
        public async Task RenderListOptions()
        {
            ...
            if (isVisible) {
                if (ModalView && !IsMobile())
                    listOption?.SetBody($@"<a class=""ew-row-link ew-view"" title=""{viewcaption}"" data-table=""mt_user"" data-caption=""{viewcaption}"" data-ew-action=""modal"" data-action=""view"" data-ajax=""" + (UseAjaxActions ? "true" : "false") + "\" data-url=\"" + HtmlEncode(AppPath(ViewUrl)) + "\" data-btn=\"null\">" + Language.Phrase("ViewLink") + "</a>");
                else
            ...
            if (isVisible) {
                if (ModalEdit && !IsMobile())
                    listOption?.SetBody($@"<a class=""ew-row-link ew-edit"" title=""{editcaption}"" data-table=""mt_user"" data-caption=""{editcaption}"" data-ew-action=""modal"" data-action=""edit"" data-ajax=""" + (UseAjaxActions ? "true" : "false") + "\" data-url=\"" + HtmlEncode(AppPath(EditUrl)) + "\" data-btn=\"SaveBtn\">" + Language.Phrase("EditLink") + "</a>");
                else
            ...
            if (isVisible) {
                if (ModalAdd && !IsMobile())
                    listOption?.SetBody($@"<a class=""ew-row-link ew-copy"" title=""{copycaption}"" data-table=""mt_user"" data-caption=""{copycaption}"" data-ew-action=""modal"" data-action=""add"" data-ajax=""" + (UseAjaxActions ? "true" : "false") + "\" data-url=\"" + HtmlEncode(AppPath(CopyUrl)) + "\" data-btn=\"AddBtn\">" + Language.Phrase("CopyLink") + "</a>");
                else
            ...
            if (listOption != null && !IsExport() && CurrentAction == "") {
                ...
                foreach (var (key, act) in ListActions.Items) {
                    if (act.Select == Config.ActionSingle && act.Allowed) {
                        ...
                        string link = "<li><button type=\"button\" class=\"dropdown-item ew-action ew-list-action\" data-caption=\"" + HtmlTitle(caption) + "\" data-ew-action=\"submit\" form=\"fmt_userlist\" data-key=\"" + KeyToJson(true) + "\"" + act.ToDataAttrs() + ">" + icon + " " + caption + "</button></li>";
                        if (!Empty(link)) {
                            links.Add(link);
                            if (Empty(body)) // Setup first button
                                body = "<button type=\"button\" class=\"btn btn-default ew-action ew-list-action\" title=\"" + HtmlTitle(caption) + "\" data-caption=\"" + HtmlTitle(caption) + "\" data-ew-action=\"submit\" form=\"fmt_userlist\" data-key=\"" + KeyToJson(true) + "\"" + act.ToDataAttrs() + ">" + icon + caption + "</button>";
                        }
                    }
                }
            ...
            listOption?.SetBody("<div class=\"form-check\"><input type=\"checkbox\" id=\"key_m_" + RowCount + "\" name=\"key_m[]\" class=\"form-check-input ew-multi-select\" value=\"" + HtmlEncode(_UserName.CurrentValue) + "\" data-ew-action=\"select-key\"></div>");
        ...
        protected void SetupOtherOptions() {
            ...
            if (ModalAdd && !IsMobile())
                item.Body = $@"<a class=""ew-add-edit ew-add"" title=""{addTitle}"" data-table=""mt_user"" data-caption=""{addTitle}"" data-ew-action=""modal"" data-action=""add"" data-ajax=""" + (UseAjaxActions ? "true" : "false") + "\" data-url=\"" + HtmlEncode(AppPath(AddUrl)) + "\" data-btn=\"AddBtn\">" + Language.Phrase("AddLink") + "</a>";
            else
            ...
            item.Body = $@"<button type=""button"" class=""ew-action ew-multi-delete"" title=""{deleteTitle}"" data-caption=""{deleteTitle}"" form=""fmt_userlist""" +
            ...
            if (UseColumnVisibility) {
                ...
                CreateColumnOption(option.Add("UserPassword")); // DN
                CreateColumnOption(option.Add("UserLevel")); // DN
                CreateColumnOption(option.Add("UserEmail")); // DN
                CreateColumnOption(option.Add("Full_Name")); // DN
                CreateColumnOption(option.Add("Position")); // DN
                CreateColumnOption(option.Add("Department")); // DN
            ...
            item.Body = "<a class=\"ew-save-filter\" data-form=\"fmt_usersrch\" data-ew-action=\"none\">" + Language.Phrase("SaveCurrentFilter") + "</a>";
            item.Visible = true;
            item = FilterOptions.Add("deletefilter");
            item.Body = "<a class=\"ew-delete-filter\" data-form=\"fmt_usersrch\" data-ew-action=\"none\">" + Language.Phrase("DeleteFilter") + "</a>";
            item.Visible = true;
        ...
        public void RenderOtherOptions()
        {
            ...
            var options = OtherOptions;
                ...
                foreach (var (key, act) in ListActions.Items.Where(kvp => kvp.Value.Select == Config.ActionMultiple)) {
                    ...
                    item.Body = "<<button type=\"button\" class=\"btn btn-default ew-action ew-list-action\" title=\"" + HtmlEncode(caption) + "\" data-caption=\"" + HtmlEncode(caption) + "\" data-ew-action=\"submit\" form=\"fmt_userlist\"" + act.ToDataAttrs() + ">" + icon + "</button>";
        ...
        public async Task SetupRow()
        {
            ...
            RowAttrs.Add("data-rowindex", ConvertToString(mtUserList.RowCount));
            RowAttrs.Add("data-key", GetKey(true));
            RowAttrs.Add("id", "r" + ConvertToString(mtUserList.RowCount) + "_mt_user");
            RowAttrs.Add("data-rowtype", ConvertToString((int)RowType));
            RowAttrs.AppendClass(mtUserList.RowCount % 2 != 1 ? "ew-table-alt-row" : "");
            if (IsAdd && mtUserList.RowType == RowType.Add || IsEdit && mtUserList.RowType == RowType.Edit) // Inline-Add/Edit row
        ...
        protected void SetupSearchOptions() {
            ...
            item.Body = "<a class=\"btn btn-default ew-search-toggle" + searchToggleClass + "\" role=\"button\" title=\"" + Language.Phrase("SearchPanel") + "\" data-caption=\"" + Language.Phrase("SearchPanel") + "\" data-ew-action=\"search-toggle\" data-form=\"fmt_usersrch\" aria-pressed=\"" + (searchToggleClass == " active" ? "true" : "false") + "\">" + Language.Phrase("SearchLink") + "</a>";
            item.Visible = true;
