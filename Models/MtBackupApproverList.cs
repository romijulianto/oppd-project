...
public partial class OverridePS {
    /// <summary>
    /// mtBackupApproverList
    /// </summary>
    public static MtBackupApproverList mtBackupApproverList
    {
        get => HttpData.Get<MtBackupApproverList>("mtBackupApproverList")!;
        set => HttpData["mtBackupApproverList"] = value;
    }

    /// <summary>
    /// Page class for mt_backup_approver
    /// </summary>
    public class MtBackupApproverList : MtBackupApproverListBase
    {
        // Constructor
        public MtBackupApproverList(Controller controller) : base(controller)
        {
        }

        // Constructor
        public MtBackupApproverList() : base()
        {
        }
    }
    ...
    /// Page base class
    /// </summary>
    public class MtBackupApproverListBase : MtBackupApprover
    {
        // Page ID
        ...
        // Page object name
        public string PageObjName = "mtBackupApproverList";

        // Title
        public string? Title = null; // Title for <title> tag

        // Grid form hidden field names
        public string FormName = "fmt_backup_approverlist";
        ...
        // Constructor
        public MtBackupApproverListBase()
        {
            // CSS class name as context
            TableVar = "mt_backup_approver";
            ...
            MultiDeleteUrl = "MtBackupApproverDelete";
            MultiUpdateUrl = "MtBackupApproverUpdate";
        ...
        // Page name
        public string PageName => "MtBackupApproverList";
        ...
        // Constructor
        public MtBackupApproverListBase(Controller? controller = null): this() { // DN
            if (controller != null)
        ...
        public override async Task<IActionResult> Run()
        {
            ...
            if (IsModal)
                FormName = "fmt_backup_approvergrid";
            ...
            if (!IsApi() && !IsTerminated) {
                ...
                // Page Render event
                mtBackupApproverList?.PageRender();
            }
            return PageResult();
        }
        #pragma warning restore 219
        ...
        public async Task<string> GetFilterList()
        {
            ...
            filters.Merge(JObject.Parse(Position.AdvancedSearch.ToJson())); // Field Position
            filters.Merge(JObject.Parse(Departement.AdvancedSearch.ToJson())); // Field Departement
            filters.Merge(JObject.Parse(Division.AdvancedSearch.ToJson())); // Field Division
            filters.Merge(JObject.Parse(Area.AdvancedSearch.ToJson())); // Field Area
            filters.Merge(JObject.Parse(Business_Area.AdvancedSearch.ToJson())); // Field Business_Area
            filters.Merge(JObject.Parse(Report_To.AdvancedSearch.ToJson())); // Field Report_To
            filters.Merge(JObject.Parse(Business_Area_Desc.AdvancedSearch.ToJson())); // Field Business_Area_Desc
        ...
        protected bool RestoreFilterList() {
            ...
            // Field Position
            if (filter?.TryGetValue("x_Position", out sv) ?? false) {
                Position.AdvancedSearch.SearchValue = sv;
                Position.AdvancedSearch.SearchOperator = filter["z_Position"];
                Position.AdvancedSearch.SearchCondition = filter["v_Position"];
                Position.AdvancedSearch.SearchValue2 = filter["y_Position"];
                Position.AdvancedSearch.SearchOperator2 = filter["w_Position"];
                Position.AdvancedSearch.Save();
            }

            // Field Departement
            if (filter?.TryGetValue("x_Departement", out sv) ?? false) {
                Departement.AdvancedSearch.SearchValue = sv;
                Departement.AdvancedSearch.SearchOperator = filter["z_Departement"];
                Departement.AdvancedSearch.SearchCondition = filter["v_Departement"];
                Departement.AdvancedSearch.SearchValue2 = filter["y_Departement"];
                Departement.AdvancedSearch.SearchOperator2 = filter["w_Departement"];
                Departement.AdvancedSearch.Save();
            }

            // Field Division
            if (filter?.TryGetValue("x_Division", out sv) ?? false) {
                Division.AdvancedSearch.SearchValue = sv;
                Division.AdvancedSearch.SearchOperator = filter["z_Division"];
                Division.AdvancedSearch.SearchCondition = filter["v_Division"];
                Division.AdvancedSearch.SearchValue2 = filter["y_Division"];
                Division.AdvancedSearch.SearchOperator2 = filter["w_Division"];
                Division.AdvancedSearch.Save();
            }

            // Field Area
            if (filter?.TryGetValue("x_Area", out sv) ?? false) {
            ...
            // Field Business_Area
            if (filter?.TryGetValue("x_Business_Area", out sv) ?? false) {
                Business_Area.AdvancedSearch.SearchValue = sv;
                Business_Area.AdvancedSearch.SearchOperator = filter["z_Business_Area"];
                Business_Area.AdvancedSearch.SearchCondition = filter["v_Business_Area"];
                Business_Area.AdvancedSearch.SearchValue2 = filter["y_Business_Area"];
                Business_Area.AdvancedSearch.SearchOperator2 = filter["w_Business_Area"];
                Business_Area.AdvancedSearch.Save();
            }

            // Field Report_To
            if (filter?.TryGetValue("x_Report_To", out sv) ?? false) {
            ...
            if (filter?.TryGetValue("x_Business_Area_Desc", out sv) ?? false) {
                Business_Area_Desc.AdvancedSearch.SearchValue = sv;
                Business_Area_Desc.AdvancedSearch.SearchOperator = filter["z_Business_Area_Desc"];
                Business_Area_Desc.AdvancedSearch.SearchCondition = filter["v_Business_Area_Desc"];
                Business_Area_Desc.AdvancedSearch.SearchValue2 = filter["y_Business_Area_Desc"];
                Business_Area_Desc.AdvancedSearch.SearchOperator2 = filter["w_Business_Area_Desc"];
                Business_Area_Desc.AdvancedSearch.Save();
            }
        ...
        public string BasicSearchWhere(bool def = false) {
            ...
            searchFlds.Add(Position);
            searchFlds.Add(Departement);
            searchFlds.Add(Division);
            searchFlds.Add(Area);
            searchFlds.Add(Business_Area);
            searchFlds.Add(Report_To);
            searchFlds.Add(Business_Area_Desc);
        ...
        protected void SetupSortOrder() {
            ...
            if (Get("order", out StringValues sv)) {
                ...
                UpdateSort(Position); // Position
                UpdateSort(Departement); // Departement
                UpdateSort(Division); // Division
                UpdateSort(Area); // Area
                UpdateSort(Business_Area); // Business_Area
                UpdateSort(Report_To); // Report_To
                UpdateSort(Business_Area_Desc); // Business_Area_Desc
        ...
        public async Task RenderListOptions()
        {
            ...
            if (isVisible) {
                if (ModalView && !IsMobile())
                    listOption?.SetBody($@"<a class=""ew-row-link ew-view"" title=""{viewcaption}"" data-table=""mt_backup_approver"" data-caption=""{viewcaption}"" data-ew-action=""modal"" data-action=""view"" data-ajax=""" + (UseAjaxActions ? "true" : "false") + "\" data-url=\"" + HtmlEncode(AppPath(ViewUrl)) + "\" data-btn=\"null\">" + Language.Phrase("ViewLink") + "</a>");
                else
            ...
            if (isVisible) {
                if (ModalEdit && !IsMobile())
                    listOption?.SetBody($@"<a class=""ew-row-link ew-edit"" title=""{editcaption}"" data-table=""mt_backup_approver"" data-caption=""{editcaption}"" data-ew-action=""modal"" data-action=""edit"" data-ajax=""" + (UseAjaxActions ? "true" : "false") + "\" data-url=\"" + HtmlEncode(AppPath(EditUrl)) + "\" data-btn=\"SaveBtn\">" + Language.Phrase("EditLink") + "</a>");
                else
            ...
            if (isVisible) {
                if (ModalAdd && !IsMobile())
                    listOption?.SetBody($@"<a class=""ew-row-link ew-copy"" title=""{copycaption}"" data-table=""mt_backup_approver"" data-caption=""{copycaption}"" data-ew-action=""modal"" data-action=""add"" data-ajax=""" + (UseAjaxActions ? "true" : "false") + "\" data-url=\"" + HtmlEncode(AppPath(CopyUrl)) + "\" data-btn=\"AddBtn\">" + Language.Phrase("CopyLink") + "</a>");
                else
            ...
            if (listOption != null && !IsExport() && CurrentAction == "") {
                ...
                foreach (var (key, act) in ListActions.Items) {
                    if (act.Select == Config.ActionSingle && act.Allowed) {
                        ...
                        string link = "<li><button type=\"button\" class=\"dropdown-item ew-action ew-list-action\" data-caption=\"" + HtmlTitle(caption) + "\" data-ew-action=\"submit\" form=\"fmt_backup_approverlist\" data-key=\"" + KeyToJson(true) + "\"" + act.ToDataAttrs() + ">" + icon + " " + caption + "</button></li>";
                        if (!Empty(link)) {
                            links.Add(link);
                            if (Empty(body)) // Setup first button
                                body = "<button type=\"button\" class=\"btn btn-default ew-action ew-list-action\" title=\"" + HtmlTitle(caption) + "\" data-caption=\"" + HtmlTitle(caption) + "\" data-ew-action=\"submit\" form=\"fmt_backup_approverlist\" data-key=\"" + KeyToJson(true) + "\"" + act.ToDataAttrs() + ">" + icon + caption + "</button>";
                        }
                    }
                }
        ...
        protected void SetupOtherOptions() {
            ...
            if (ModalAdd && !IsMobile())
                item.Body = $@"<a class=""ew-add-edit ew-add"" title=""{addTitle}"" data-table=""mt_backup_approver"" data-caption=""{addTitle}"" data-ew-action=""modal"" data-action=""add"" data-ajax=""" + (UseAjaxActions ? "true" : "false") + "\" data-url=\"" + HtmlEncode(AppPath(AddUrl)) + "\" data-btn=\"AddBtn\">" + Language.Phrase("AddLink") + "</a>";
            else
            ...
            item.Body = $@"<button type=""button"" class=""ew-action ew-multi-delete"" title=""{deleteTitle}"" data-caption=""{deleteTitle}"" form=""fmt_backup_approverlist""" +
            ...
            if (UseColumnVisibility) {
                ...
                CreateColumnOption(option.Add("Position")); // DN
                CreateColumnOption(option.Add("Departement")); // DN
                CreateColumnOption(option.Add("Division")); // DN
                CreateColumnOption(option.Add("Area")); // DN
                CreateColumnOption(option.Add("Business_Area")); // DN
                CreateColumnOption(option.Add("Report_To")); // DN
                CreateColumnOption(option.Add("Business_Area_Desc")); // DN
            }
            ...
            item.Body = "<a class=\"ew-save-filter\" data-form=\"fmt_backup_approversrch\" data-ew-action=\"none\">" + Language.Phrase("SaveCurrentFilter") + "</a>";
            item.Visible = true;
            item = FilterOptions.Add("deletefilter");
            item.Body = "<a class=\"ew-delete-filter\" data-form=\"fmt_backup_approversrch\" data-ew-action=\"none\">" + Language.Phrase("DeleteFilter") + "</a>";
            item.Visible = true;
        ...
        public void RenderOtherOptions()
        {
            ...
            var options = OtherOptions;
                ...
                foreach (var (key, act) in ListActions.Items.Where(kvp => kvp.Value.Select == Config.ActionMultiple)) {
                    ...
                    item.Body = "<<button type=\"button\" class=\"btn btn-default ew-action ew-list-action\" title=\"" + HtmlEncode(caption) + "\" data-caption=\"" + HtmlEncode(caption) + "\" data-ew-action=\"submit\" form=\"fmt_backup_approverlist\"" + act.ToDataAttrs() + ">" + icon + "</button>";
        ...
        public async Task SetupRow()
        {
            if (IsGridAdd || IsGridEdit) {
                if (SameString(RowIndex, "$rowindex$")) { // Render template row first
                    ...
                    RowAttrs.Add("id", "r0_mt_backup_approver");
            ...
            RowAttrs.Add("data-rowindex", ConvertToString(mtBackupApproverList.RowCount));
            RowAttrs.Add("data-key", GetKey(true));
            RowAttrs.Add("id", "r" + ConvertToString(mtBackupApproverList.RowCount) + "_mt_backup_approver");
            RowAttrs.Add("data-rowtype", ConvertToString((int)RowType));
            RowAttrs.AppendClass(mtBackupApproverList.RowCount % 2 != 1 ? "ew-table-alt-row" : "");
            if (IsAdd && mtBackupApproverList.RowType == RowType.Add || IsEdit && mtBackupApproverList.RowType == RowType.Edit) // Inline-Add/Edit row
        ...
        public async Task RenderRow()
        {
            ...
            if (RowType == RowType.View) {
                ...
                Business_Area_Desc.TooltipValue = "";
            }
        ...
        protected void SetupSearchOptions() {
            ...
            item.Body = "<a class=\"btn btn-default ew-search-toggle" + searchToggleClass + "\" role=\"button\" title=\"" + Language.Phrase("SearchPanel") + "\" data-caption=\"" + Language.Phrase("SearchPanel") + "\" data-ew-action=\"search-toggle\" data-form=\"fmt_backup_approversrch\" aria-pressed=\"" + (searchToggleClass == " active" ? "true" : "false") + "\">" + Language.Phrase("SearchLink") + "</a>";
            item.Visible = true;
