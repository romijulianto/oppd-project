...
public partial class OverridePS {
    ...
    /// trRequestEdit
    /// </summary>
    public static TrRequestEdit trRequestEdit
    {
        get => HttpData.Get<TrRequestEdit>("trRequestEdit")!;
        set => HttpData["trRequestEdit"] = value;
    }

    /// <summary>
    /// Page class for tr_request
    /// </summary>
    public class TrRequestEdit : TrRequestEditBase
    {
        // Constructor
        public TrRequestEdit(Controller controller) : base(controller)
        {
        }

        // Constructor
        public TrRequestEdit() : base()
        {
        }
    }
    ...
    /// Page base class
    /// </summary>
    public class TrRequestEditBase : TrRequest
    {
        // Page ID
        ...
        // Page object name
        public string PageObjName = "trRequestEdit";
        ...
        // Page name
        public string PageName => "TrRequestEdit";
        ...
        // Constructor
        public TrRequestEditBase(Controller? controller = null): this() { // DN
            if (controller != null)
        ...
        switch (CurrentAction) {
                case "show": // Get a record to display
                        if (!loaded) { // Load record based on key
                            ...
                            return Terminate("TrRequestList"); // No matching record, return to list
                        }
        ...
        protected async Task<bool> ValidateForm() {
            ...
            if (Request_By.Required) {
                if (!Request_By.IsDetailKey && Empty(Request_By.FormValue)) {
            ...
            if (Request_By_Name.Required) {
                if (!Request_By_Name.IsDetailKey && Empty(Request_By_Name.FormValue)) {
            ...
            if (Request_By_Position.Required) {
                if (!Request_By_Position.IsDetailKey && Empty(Request_By_Position.FormValue)) {
            ...
            if (detailTblVar.Contains("tr_request_detail") && trRequestDetail?.DetailEdit == true) {
            ...
            if (detailTblVar.Contains("tr_request_approver") && trRequestApprover?.DetailEdit == true) {
            ...
            if (detailTblVar.Contains("tr_request_approval_history") && trRequestApprovalHistory?.DetailEdit == true) {
        ...
        protected async Task<JsonBoolResult> EditRow() { // DN
            ...
            // Request_No
            Request_No.SetDbValue(rsnew, Request_No.CurrentValue, Request_No.ReadOnly);

            // Reference_Doc
            Reference_Doc.SetDbValue(rsnew, Reference_Doc.CurrentValue, Reference_Doc.ReadOnly);
            ...
            // Request_By_Branch
            Request_By_Branch.SetDbValue(rsnew, Request_By_Branch.CurrentValue, Request_By_Branch.ReadOnly);

            // Request_By_Region
            Request_By_Region.SetDbValue(rsnew, Request_By_Region.CurrentValue, Request_By_Region.ReadOnly);

            // Request_Industry
            Request_Industry.SetDbValue(rsnew, Request_Industry.CurrentValue, Request_Industry.ReadOnly);

            // Customer_ID
            Customer_ID.SetDbValue(rsnew, Customer_ID.CurrentValue, Customer_ID.ReadOnly);

            // Customer_Name
            Customer_Name.SetDbValue(rsnew, Customer_Name.CurrentValue, Customer_Name.ReadOnly);

            // SAP_Total
            SAP_Total.SetDbValue(rsnew, SAP_Total.CurrentValue, SAP_Total.ReadOnly);

            // Override_Total
            Override_Total.SetDbValue(rsnew, Override_Total.CurrentValue, Override_Total.ReadOnly);

            // Variance_Total
            Variance_Total.SetDbValue(rsnew, Variance_Total.CurrentValue, Variance_Total.ReadOnly);

            // Variance_Percent
            Variance_Percent.SetDbValue(rsnew, Variance_Percent.CurrentValue, Variance_Percent.ReadOnly);

            // Notes
            Notes.SetDbValue(rsnew, Notes.CurrentValue, Notes.ReadOnly);

            // Next_Approver
            Next_Approver.SetDbValue(rsnew, Next_Approver.CurrentValue, Next_Approver.ReadOnly);

            // Request_PIC
            Request_PIC.SetDbValue(rsnew, Request_PIC.CurrentValue, Request_PIC.ReadOnly);

            // Request_Status
            Request_Status.SetDbValue(rsnew, Request_Status.CurrentValue, Request_Status.ReadOnly);

            // List_Approver
            List_Approver.SetDbValue(rsnew, List_Approver.CurrentValue, List_Approver.ReadOnly);
            ...
            // Request_Currency
            Request_Currency.SetDbValue(rsnew, Request_Currency.CurrentValue, Request_Currency.ReadOnly);
            ...
            if (updateRow) {
                try {
                    ...
                    var detailTblVar = CurrentDetailTables;
                    if (detailTblVar.Contains("tr_request_detail") && trRequestDetail?.DetailEdit == true && result) {
                        trRequestDetailGrid = Resolve("TrRequestDetailGrid")!; // Get detail page object
                        if (trRequestDetailGrid != null) {
                            Security.LoadCurrentUserLevel(ProjectID + "tr_request_detail"); // Load user level of detail table
                            result = await trRequestDetailGrid.GridUpdate();
                            Security.LoadCurrentUserLevel(ProjectID + TableName); // Restore user level of master table
                        }
                    }
                    if (detailTblVar.Contains("tr_request_approver") && trRequestApprover?.DetailEdit == true && result) {
                        trRequestApproverGrid = Resolve("TrRequestApproverGrid")!; // Get detail page object
                        if (trRequestApproverGrid != null) {
                            Security.LoadCurrentUserLevel(ProjectID + "tr_request_approver"); // Load user level of detail table
                            result = await trRequestApproverGrid.GridUpdate();
                            Security.LoadCurrentUserLevel(ProjectID + TableName); // Restore user level of master table
                        }
                    }
                    if (detailTblVar.Contains("tr_request_approval_history") && trRequestApprovalHistory?.DetailEdit == true && result) {
                        trRequestApprovalHistoryGrid = Resolve("TrRequestApprovalHistoryGrid")!; // Get detail page object
                        if (trRequestApprovalHistoryGrid != null) {
                            Security.LoadCurrentUserLevel(ProjectID + "tr_request_approval_history"); // Load user level of detail table
                            result = await trRequestApprovalHistoryGrid.GridUpdate();
                            Security.LoadCurrentUserLevel(ProjectID + TableName); // Restore user level of master table
                        }
                    }
        ...
        protected void SetupDetailParms() {
            ...
            if (!Empty(detailTblVar)) {
                ...
                if (detailTblVars.Contains("tr_request_detail")) {
                    ...
                    if (trRequestDetailGrid?.DetailEdit ?? false) {
                        trRequestDetailGrid.CurrentMode = "edit";
                        trRequestDetailGrid.CurrentAction = "gridedit";
                ...
                if (detailTblVars.Contains("tr_request_approver")) {
                    ...
                    if (trRequestApproverGrid?.DetailEdit ?? false) {
                        trRequestApproverGrid.CurrentMode = "edit";
                        trRequestApproverGrid.CurrentAction = "gridedit";
                ...
                if (detailTblVars.Contains("tr_request_approval_history")) {
                    ...
                    if (trRequestApprovalHistoryGrid?.DetailEdit ?? false) {
                        trRequestApprovalHistoryGrid.CurrentMode = "edit";
                        trRequestApprovalHistoryGrid.CurrentAction = "gridedit";
