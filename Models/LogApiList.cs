...
public partial class OverridePS {
    ...
    /// logApiList
    /// </summary>
    public static LogApiList logApiList
    {
        get => HttpData.Get<LogApiList>("logApiList")!;
        set => HttpData["logApiList"] = value;
    }

    /// <summary>
    /// Page class for log_API
    /// </summary>
    public class LogApiList : LogApiListBase
    {
        // Constructor
        public LogApiList(Controller controller) : base(controller)
        {
        }

        // Constructor
        public LogApiList() : base()
        {
        }
    }
    ...
    /// Page base class
    /// </summary>
    public class LogApiListBase : LogApi
    {
        // Page ID
        ...
        // Page object name
        public string PageObjName = "logApiList";

        // Title
        public string? Title = null; // Title for <title> tag

        // Grid form hidden field names
        public string FormName = "flog_APIlist";
        ...
        public LogApiListBase()
        {
            ...
            MultiDeleteUrl = "LogApiDelete";
            MultiUpdateUrl = "LogApiUpdate";
        ...
        // Page name
        public string PageName => "LogApiList";
        ...
        // Constructor
        public LogApiListBase(Controller? controller = null): this() { // DN
            if (controller != null)
        ...
        public async Task<string> GetFilterList()
        {
            ...
            filters.Merge(JObject.Parse(API_Type.AdvancedSearch.ToJson())); // Field API_Type
            filters.Merge(JObject.Parse(API_Link.AdvancedSearch.ToJson())); // Field API_Link
            filters.Merge(JObject.Parse(API_Body.AdvancedSearch.ToJson())); // Field API_Body
            filters.Merge(JObject.Parse(API_Status.AdvancedSearch.ToJson())); // Field API_Status
            filters.Merge(JObject.Parse(API_Result.AdvancedSearch.ToJson())); // Field API_Result
        ...
        protected bool RestoreFilterList() {
            ...
            // Field API_Type
            if (filter?.TryGetValue("x_API_Type", out sv) ?? false) {
                API_Type.AdvancedSearch.SearchValue = sv;
                API_Type.AdvancedSearch.SearchOperator = filter["z_API_Type"];
                API_Type.AdvancedSearch.SearchCondition = filter["v_API_Type"];
                API_Type.AdvancedSearch.SearchValue2 = filter["y_API_Type"];
                API_Type.AdvancedSearch.SearchOperator2 = filter["w_API_Type"];
                API_Type.AdvancedSearch.Save();
            }

            // Field API_Link
            if (filter?.TryGetValue("x_API_Link", out sv) ?? false) {
                API_Link.AdvancedSearch.SearchValue = sv;
                API_Link.AdvancedSearch.SearchOperator = filter["z_API_Link"];
                API_Link.AdvancedSearch.SearchCondition = filter["v_API_Link"];
                API_Link.AdvancedSearch.SearchValue2 = filter["y_API_Link"];
                API_Link.AdvancedSearch.SearchOperator2 = filter["w_API_Link"];
                API_Link.AdvancedSearch.Save();
            }

            // Field API_Body
            if (filter?.TryGetValue("x_API_Body", out sv) ?? false) {
                API_Body.AdvancedSearch.SearchValue = sv;
                API_Body.AdvancedSearch.SearchOperator = filter["z_API_Body"];
                API_Body.AdvancedSearch.SearchCondition = filter["v_API_Body"];
                API_Body.AdvancedSearch.SearchValue2 = filter["y_API_Body"];
                API_Body.AdvancedSearch.SearchOperator2 = filter["w_API_Body"];
                API_Body.AdvancedSearch.Save();
            }

            // Field API_Status
            if (filter?.TryGetValue("x_API_Status", out sv) ?? false) {
                API_Status.AdvancedSearch.SearchValue = sv;
                API_Status.AdvancedSearch.SearchOperator = filter["z_API_Status"];
                API_Status.AdvancedSearch.SearchCondition = filter["v_API_Status"];
                API_Status.AdvancedSearch.SearchValue2 = filter["y_API_Status"];
                API_Status.AdvancedSearch.SearchOperator2 = filter["w_API_Status"];
                API_Status.AdvancedSearch.Save();
            }

            // Field API_Result
            if (filter?.TryGetValue("x_API_Result", out sv) ?? false) {
                API_Result.AdvancedSearch.SearchValue = sv;
                API_Result.AdvancedSearch.SearchOperator = filter["z_API_Result"];
                API_Result.AdvancedSearch.SearchCondition = filter["v_API_Result"];
                API_Result.AdvancedSearch.SearchValue2 = filter["y_API_Result"];
                API_Result.AdvancedSearch.SearchOperator2 = filter["w_API_Result"];
                API_Result.AdvancedSearch.Save();
            }
        ...
        public string BasicSearchWhere(bool def = false) {
            ...
            searchFlds.Add(API_Type);
            searchFlds.Add(API_Link);
            searchFlds.Add(API_Body);
            searchFlds.Add(API_Status);
            searchFlds.Add(API_Result);
        ...
        protected void SetupSortOrder() {
            ...
            if (Get("order", out StringValues sv)) {
                ...
                UpdateSort(id); // id
                UpdateSort(API_Type); // API_Type
                UpdateSort(API_Link); // API_Link
                UpdateSort(API_Body); // API_Body
                UpdateSort(API_Status); // API_Status
                UpdateSort(API_Result); // API_Result
        ...
        public async Task RenderListOptions()
        {
            ...
            if (isVisible) {
                if (ModalView && !IsMobile())
                    listOption?.SetBody($@"<a class=""ew-row-link ew-view"" title=""{viewcaption}"" data-table=""log_API"" data-caption=""{viewcaption}"" data-ew-action=""modal"" data-action=""view"" data-ajax=""" + (UseAjaxActions ? "true" : "false") + "\" data-url=\"" + HtmlEncode(AppPath(ViewUrl)) + "\" data-btn=\"null\">" + Language.Phrase("ViewLink") + "</a>");
                else
            ...
            if (isVisible) {
                if (ModalEdit && !IsMobile())
                    listOption?.SetBody($@"<a class=""ew-row-link ew-edit"" title=""{editcaption}"" data-table=""log_API"" data-caption=""{editcaption}"" data-ew-action=""modal"" data-action=""edit"" data-ajax=""" + (UseAjaxActions ? "true" : "false") + "\" data-url=\"" + HtmlEncode(AppPath(EditUrl)) + "\" data-btn=\"SaveBtn\">" + Language.Phrase("EditLink") + "</a>");
                else
            ...
            if (isVisible) {
                if (ModalAdd && !IsMobile())
                    listOption?.SetBody($@"<a class=""ew-row-link ew-copy"" title=""{copycaption}"" data-table=""log_API"" data-caption=""{copycaption}"" data-ew-action=""modal"" data-action=""add"" data-ajax=""" + (UseAjaxActions ? "true" : "false") + "\" data-url=\"" + HtmlEncode(AppPath(CopyUrl)) + "\" data-btn=\"AddBtn\">" + Language.Phrase("CopyLink") + "</a>");
                else
            ...
            if (listOption != null && !IsExport() && CurrentAction == "") {
                ...
                foreach (var (key, act) in ListActions.Items) {
                    if (act.Select == Config.ActionSingle && act.Allowed) {
                        ...
                        string link = "<li><button type=\"button\" class=\"dropdown-item ew-action ew-list-action\" data-caption=\"" + HtmlTitle(caption) + "\" data-ew-action=\"submit\" form=\"flog_APIlist\" data-key=\"" + KeyToJson(true) + "\"" + act.ToDataAttrs() + ">" + icon + " " + caption + "</button></li>";
                        if (!Empty(link)) {
                            links.Add(link);
                            if (Empty(body)) // Setup first button
                                body = "<button type=\"button\" class=\"btn btn-default ew-action ew-list-action\" title=\"" + HtmlTitle(caption) + "\" data-caption=\"" + HtmlTitle(caption) + "\" data-ew-action=\"submit\" form=\"flog_APIlist\" data-key=\"" + KeyToJson(true) + "\"" + act.ToDataAttrs() + ">" + icon + caption + "</button>";
                        }
                    }
                }
        ...
        protected void SetupOtherOptions() {
            ...
            if (ModalAdd && !IsMobile())
                item.Body = $@"<a class=""ew-add-edit ew-add"" title=""{addTitle}"" data-table=""log_API"" data-caption=""{addTitle}"" data-ew-action=""modal"" data-action=""add"" data-ajax=""" + (UseAjaxActions ? "true" : "false") + "\" data-url=\"" + HtmlEncode(AppPath(AddUrl)) + "\" data-btn=\"AddBtn\">" + Language.Phrase("AddLink") + "</a>";
            else
            ...
            item.Body = $@"<button type=""button"" class=""ew-action ew-multi-delete"" title=""{deleteTitle}"" data-caption=""{deleteTitle}"" form=""flog_APIlist""" +
            ...
            if (UseColumnVisibility) {
                ...
                CreateColumnOption(option.Add("API_Type")); // DN
                CreateColumnOption(option.Add("API_Link")); // DN
                CreateColumnOption(option.Add("API_Body")); // DN
                CreateColumnOption(option.Add("API_Status")); // DN
                CreateColumnOption(option.Add("API_Result")); // DN
            }
            ...
            item.Body = "<a class=\"ew-save-filter\" data-form=\"flog_APIsrch\" data-ew-action=\"none\">" + Language.Phrase("SaveCurrentFilter") + "</a>";
            item.Visible = true;
            item = FilterOptions.Add("deletefilter");
            item.Body = "<a class=\"ew-delete-filter\" data-form=\"flog_APIsrch\" data-ew-action=\"none\">" + Language.Phrase("DeleteFilter") + "</a>";
            item.Visible = true;
        ...
        public void RenderOtherOptions()
        {
            ...
            var options = OtherOptions;
                ...
                foreach (var (key, act) in ListActions.Items.Where(kvp => kvp.Value.Select == Config.ActionMultiple)) {
                    ...
                    item.Body = "<<button type=\"button\" class=\"btn btn-default ew-action ew-list-action\" title=\"" + HtmlEncode(caption) + "\" data-caption=\"" + HtmlEncode(caption) + "\" data-ew-action=\"submit\" form=\"flog_APIlist\"" + act.ToDataAttrs() + ">" + icon + "</button>";
        ...
        public async Task SetupRow()
        {
            ...
            RowAttrs.Add("data-rowindex", ConvertToString(logApiList.RowCount));
            RowAttrs.Add("data-key", GetKey(true));
            RowAttrs.Add("id", "r" + ConvertToString(logApiList.RowCount) + "_log_API");
            RowAttrs.Add("data-rowtype", ConvertToString((int)RowType));
            RowAttrs.AppendClass(logApiList.RowCount % 2 != 1 ? "ew-table-alt-row" : "");
            if (IsAdd && logApiList.RowType == RowType.Add || IsEdit && logApiList.RowType == RowType.Edit) // Inline-Add/Edit row
        ...
        public async Task RenderRow()
        {
            ...
            if (RowType == RowType.View) {
                ...
                // API_Type
                API_Type.HrefValue = "";
                API_Type.TooltipValue = "";

                // API_Link
                API_Link.HrefValue = "";
                API_Link.TooltipValue = "";

                // API_Body
                API_Body.HrefValue = "";
                API_Body.TooltipValue = "";

                // API_Status
                API_Status.HrefValue = "";
                API_Status.TooltipValue = "";

                // API_Result
                API_Result.HrefValue = "";
                API_Result.TooltipValue = "";
            }
        ...
        protected void SetupSearchOptions() {
            ...
            item.Body = "<a class=\"btn btn-default ew-search-toggle" + searchToggleClass + "\" role=\"button\" title=\"" + Language.Phrase("SearchPanel") + "\" data-caption=\"" + Language.Phrase("SearchPanel") + "\" data-ew-action=\"search-toggle\" data-form=\"flog_APIsrch\" aria-pressed=\"" + (searchToggleClass == " active" ? "true" : "false") + "\">" + Language.Phrase("SearchLink") + "</a>";
            item.Visible = true;
