...
public partial class OverridePS {
    ...
    /// trRequestList
    /// </summary>
    public static TrRequestList trRequestList
    {
        get => HttpData.Get<TrRequestList>("trRequestList")!;
        set => HttpData["trRequestList"] = value;
    }

    /// <summary>
    /// Page class for tr_request
    /// </summary>
    public class TrRequestList : TrRequestListBase
    {
        // Constructor
        public TrRequestList(Controller controller) : base(controller)
        {
        }

        // Constructor
        public TrRequestList() : base()
        {
        }
        ...
        public override void PageLoad() {
            //Log("Page Load");
            //SetFixedHeaderTable(true, "vh-60"); // Enable fixed header table and set the height to 50vh
        }
    }
    ...
    /// Page base class
    /// </summary>
    public class TrRequestListBase : TrRequest
    {
        // Page ID
        ...
        // Page object name
        public string PageObjName = "trRequestList";

        // Title
        public string? Title = null; // Title for <title> tag

        // Grid form hidden field names
        public string FormName = "ftr_requestlist";
        ...
        public TrRequestListBase()
        {
            ...
            // Initialize URLs
            AddUrl = "TrRequestAdd?" + Config.TableShowDetail + "=";
            ...
            MultiDeleteUrl = "TrRequestDelete";
            MultiUpdateUrl = "TrRequestUpdate";
        ...
        // Page name
        public string PageName => "TrRequestList";
        ...
        // Constructor
        public TrRequestListBase(Controller? controller = null): this() { // DN
            if (controller != null)
        ...
        public async Task<string> GetFilterList()
        {
            ...
            filters.Merge(JObject.Parse(Request_Currency.AdvancedSearch.ToJson())); // Field Request_Currency
        ...
        protected bool RestoreFilterList() {
            ...
            // Field Request_Currency
            if (filter?.TryGetValue("x_Request_Currency", out sv) ?? false) {
                Request_Currency.AdvancedSearch.SearchValue = sv;
                Request_Currency.AdvancedSearch.SearchOperator = filter["z_Request_Currency"];
                Request_Currency.AdvancedSearch.SearchCondition = filter["v_Request_Currency"];
                Request_Currency.AdvancedSearch.SearchValue2 = filter["y_Request_Currency"];
                Request_Currency.AdvancedSearch.SearchOperator2 = filter["w_Request_Currency"];
                Request_Currency.AdvancedSearch.Save();
            }
        ...
        public string AdvancedSearchWhere(bool def = false) {
            ...
            BuildSearchSql(ref where, Request_No, def, true); // Request_No
            BuildSearchSql(ref where, Reference_Doc, def, true); // Reference_Doc
            BuildSearchSql(ref where, Reason, def, true); // Reason
            BuildSearchSql(ref where, Request_By, def, true); // Request_By
            BuildSearchSql(ref where, Request_By_Name, def, true); // Request_By_Name
            BuildSearchSql(ref where, Request_By_Position, def, true); // Request_By_Position
            BuildSearchSql(ref where, Request_By_Branch, def, true); // Request_By_Branch
            BuildSearchSql(ref where, Request_By_Region, def, true); // Request_By_Region
            BuildSearchSql(ref where, Request_Industry, def, true); // Request_Industry
            BuildSearchSql(ref where, Customer_ID, def, true); // Customer_ID
            BuildSearchSql(ref where, Customer_Name, def, true); // Customer_Name
            BuildSearchSql(ref where, SAP_Total, def, true); // SAP_Total
            BuildSearchSql(ref where, Override_Total, def, true); // Override_Total
            BuildSearchSql(ref where, Variance_Total, def, true); // Variance_Total
            BuildSearchSql(ref where, Variance_Percent, def, true); // Variance_Percent
            BuildSearchSql(ref where, Notes, def, true); // Notes
            BuildSearchSql(ref where, Next_Approver, def, true); // Next_Approver
            BuildSearchSql(ref where, Request_PIC, def, true); // Request_PIC
            BuildSearchSql(ref where, Request_Status, def, true); // Request_Status
            BuildSearchSql(ref where, List_Approver, def, true); // List_Approver
            ...
            BuildSearchSql(ref where, Request_Currency, def, false); // Request_Currency
            ...
            if (!def && Command == "search") {
                ...
                Request_Currency.AdvancedSearch.Save(); // Request_Currency
        ...
        public void ShowFilterList()
        {
            ...
            if (Empty(filter))
                BuildSearchSql(ref filter, Request_No, false, true);
            if (!Empty(filter))
            ...
            if (Empty(filter))
                BuildSearchSql(ref filter, Reference_Doc, false, true);
            if (!Empty(filter))
            ...
            if (Empty(filter))
                BuildSearchSql(ref filter, Reason, false, true);
            if (!Empty(filter))
            ...
            if (Empty(filter))
                BuildSearchSql(ref filter, Request_By, false, true);
            if (!Empty(filter))
            ...
            if (Empty(filter))
                BuildSearchSql(ref filter, Request_By_Name, false, true);
            if (!Empty(filter))
            ...
            if (Empty(filter))
                BuildSearchSql(ref filter, Request_By_Position, false, true);
            if (!Empty(filter))
            ...
            if (Empty(filter))
                BuildSearchSql(ref filter, Request_By_Branch, false, true);
            if (!Empty(filter))
            ...
            if (Empty(filter))
                BuildSearchSql(ref filter, Request_By_Region, false, true);
            if (!Empty(filter))
            ...
            if (Empty(filter))
                BuildSearchSql(ref filter, Request_Industry, false, true);
            if (!Empty(filter))
            ...
            if (Empty(filter))
                BuildSearchSql(ref filter, Customer_ID, false, true);
            if (!Empty(filter))
            ...
            if (Empty(filter))
                BuildSearchSql(ref filter, Customer_Name, false, true);
            if (!Empty(filter))
            ...
            if (Empty(filter))
                BuildSearchSql(ref filter, SAP_Total, false, true);
            if (!Empty(filter))
            ...
            if (Empty(filter))
                BuildSearchSql(ref filter, Override_Total, false, true);
            if (!Empty(filter))
            ...
            if (Empty(filter))
                BuildSearchSql(ref filter, Variance_Total, false, true);
            if (!Empty(filter))
            ...
            if (Empty(filter))
                BuildSearchSql(ref filter, Variance_Percent, false, true);
            if (!Empty(filter))
            ...
            if (Empty(filter))
                BuildSearchSql(ref filter, Next_Approver, false, true);
            if (!Empty(filter))
            ...
            if (Empty(filter))
                BuildSearchSql(ref filter, Request_PIC, false, true);
            if (!Empty(filter))
            ...
            if (Empty(filter))
                BuildSearchSql(ref filter, Request_Status, false, true);
            if (!Empty(filter))
            ...
            if (Empty(filter))
                BuildSearchSql(ref filter, List_Approver, false, true);
            if (!Empty(filter))
            ...
            // Field Request_Currency
            filter = QueryBuilderWhere("Request_Currency");
            if (Empty(filter))
                BuildSearchSql(ref filter, Request_Currency, false, false);
            if (!Empty(filter))
                filterList += "<div><span class=\"" + captionClass + "\">" + Request_Currency.Caption + "</span>" + captionSuffix + filter + "</div>";
            if (!Empty(BasicSearch.Keyword))
        ...
        public string BasicSearchWhere(bool def = false) {
            ...
            searchFlds.Add(Request_Currency);
        ...
        protected bool CheckSearchParms() {
            ...
            if (Created_Date.AdvancedSearch.IssetSession)
                return true;
            if (Request_Currency.AdvancedSearch.IssetSession)
                return true;
            return false;
        }
        ...
        protected void ResetAdvancedSearchParms() {
            ...
            Request_Currency.AdvancedSearch.UnsetSession();
        }
        ...
        protected void RestoreSearchParms() {
            ...
            Request_Currency.AdvancedSearch.Load();
        }
        ...
        protected void SetupSortOrder() {
            ...
            if (Get("order", out StringValues sv)) {
                ...
                UpdateSort(Request_Currency); // Request_Currency
        ...
        public async Task RenderListOptions()
        {
            ...
            if (isVisible) {
                if (ModalView && !IsMobile())
                    listOption?.SetBody($@"<a class=""ew-row-link ew-view"" title=""{viewcaption}"" data-table=""tr_request"" data-caption=""{viewcaption}"" data-ew-action=""modal"" data-action=""view"" data-ajax=""" + (UseAjaxActions ? "true" : "false") + "\" data-url=\"" + HtmlEncode(AppPath(ViewUrl)) + "\" data-btn=\"null\">" + Language.Phrase("ViewLink") + "</a>");
                else
            ...
            if (isVisible) {
                if (ModalEdit && !IsMobile())
                    listOption?.SetBody($@"<a class=""ew-row-link ew-edit"" title=""{editcaption}"" data-table=""tr_request"" data-caption=""{editcaption}"" data-ew-action=""modal"" data-action=""edit"" data-ajax=""" + (UseAjaxActions ? "true" : "false") + "\" data-url=\"" + HtmlEncode(AppPath(EditUrl)) + "\" data-btn=\"SaveBtn\">" + Language.Phrase("EditLink") + "</a>");
                else
            ...
            if (isVisible) {
                if (ModalAdd && !IsMobile())
                    listOption?.SetBody($@"<a class=""ew-row-link ew-copy"" title=""{copycaption}"" data-table=""tr_request"" data-caption=""{copycaption}"" data-ew-action=""modal"" data-action=""add"" data-ajax=""" + (UseAjaxActions ? "true" : "false") + "\" data-url=\"" + HtmlEncode(AppPath(CopyUrl)) + "\" data-btn=\"AddBtn\">" + Language.Phrase("CopyLink") + "</a>");
                else
            ...
            if (listOption != null && !IsExport() && CurrentAction == "") {
                ...
                foreach (var (key, act) in ListActions.Items) {
                    if (act.Select == Config.ActionSingle && act.Allowed) {
                        ...
                        string link = "<li><button type=\"button\" class=\"dropdown-item ew-action ew-list-action\" data-caption=\"" + HtmlTitle(caption) + "\" data-ew-action=\"submit\" form=\"ftr_requestlist\" data-key=\"" + KeyToJson(true) + "\"" + act.ToDataAttrs() + ">" + icon + " " + caption + "</button></li>";
                        if (!Empty(link)) {
                            links.Add(link);
                            if (Empty(body)) // Setup first button
                                body = "<button type=\"button\" class=\"btn btn-default ew-action ew-list-action\" title=\"" + HtmlTitle(caption) + "\" data-caption=\"" + HtmlTitle(caption) + "\" data-ew-action=\"submit\" form=\"ftr_requestlist\" data-key=\"" + KeyToJson(true) + "\"" + act.ToDataAttrs() + ">" + icon + caption + "</button>";
                        }
                    }
                }
            ...
            if (isVisible) {
                ...
                body = "<a class=\"btn btn-default ew-row-link ew-detail" + (ListOptions.UseDropDownButton ? " dropdown-toggle" : "") + "\" data-action=\"list\" href=\"" + HtmlEncode(AppPath("TrRequestDetailList?" + Config.TableShowMaster + "=tr_request&" + ForeignKeyUrl("fk_id", id.CurrentValue) + "")) + "\">" + body + "</a>";
                string links = "";
                ...
                if (detailPage?.DetailEdit && Security.CanEdit && Security.AllowEdit(CurrentProjectID + "tr_request") ?? false) {
                    ...
                    url = GetEditUrl(Config.TableShowDetail + "=tr_request_detail");
                ...
                if (detailPage?.DetailAdd && Security.CanAdd && Security.AllowAdd(CurrentProjectID + "tr_request") ?? false) {
                    ...
                    url = GetCopyUrl(Config.TableShowDetail + "=tr_request_detail");
            ...
            if (isVisible) {
                ...
                body = "<a class=\"btn btn-default ew-row-link ew-detail" + (ListOptions.UseDropDownButton ? " dropdown-toggle" : "") + "\" data-action=\"list\" href=\"" + HtmlEncode(AppPath("TrRequestApproverList?" + Config.TableShowMaster + "=tr_request&" + ForeignKeyUrl("fk_id", id.CurrentValue) + "")) + "\">" + body + "</a>";
                string links = "";
                ...
                if (detailPage?.DetailEdit && Security.CanEdit && Security.AllowEdit(CurrentProjectID + "tr_request") ?? false) {
                    caption = Language.Phrase("MasterDetailEditLink", null);
                    title = Language.Phrase("MasterDetailEditLink", true);
                    url = GetEditUrl(Config.TableShowDetail + "=tr_request_approver");
                    links += "<li><a class=\"dropdown-item ew-row-link ew-detail-edit\" data-action=\"edit\" data-caption=\"" + title + "\" href=\"" + HtmlEncode(AppPath(url)) + "\">" + caption + "</a></li>";
                    if (!Empty(detailEditTblVar))
                        detailEditTblVar += ",";
                ...
                if (detailPage?.DetailAdd && Security.CanAdd && Security.AllowAdd(CurrentProjectID + "tr_request") ?? false) {
                    caption = Language.Phrase("MasterDetailCopyLink", null);
                    title = Language.Phrase("MasterDetailCopyLink", true);
                    url = GetCopyUrl(Config.TableShowDetail + "=tr_request_approver");
                    links += "<li><a class=\"dropdown-item ew-row-link ew-detail-copy\" data-action=\"add\" data-caption=\"" + title + "\" href=\"" + HtmlEncode(AppPath(url)) + "\">" + caption + "</a></li>";
                    if (!Empty(detailCopyTblVar))
                        detailCopyTblVar += ",";
            ...
            if (isVisible) {
                ...
                body = "<a class=\"btn btn-default ew-row-link ew-detail" + (ListOptions.UseDropDownButton ? " dropdown-toggle" : "") + "\" data-action=\"list\" href=\"" + HtmlEncode(AppPath("TrRequestApprovalHistoryList?" + Config.TableShowMaster + "=tr_request&" + ForeignKeyUrl("fk_id", id.CurrentValue) + "")) + "\">" + body + "</a>";
                string links = "";
                ...
                if (detailPage?.DetailEdit && Security.CanEdit && Security.AllowEdit(CurrentProjectID + "tr_request") ?? false) {
                    caption = Language.Phrase("MasterDetailEditLink", null);
                    title = Language.Phrase("MasterDetailEditLink", true);
                    url = GetEditUrl(Config.TableShowDetail + "=tr_request_approval_history");
                    links += "<li><a class=\"dropdown-item ew-row-link ew-detail-edit\" data-action=\"edit\" data-caption=\"" + title + "\" href=\"" + HtmlEncode(AppPath(url)) + "\">" + caption + "</a></li>";
                    if (!Empty(detailEditTblVar))
                        detailEditTblVar += ",";
                ...
                if (detailPage?.DetailAdd && Security.CanAdd && Security.AllowAdd(CurrentProjectID + "tr_request") ?? false) {
                    caption = Language.Phrase("MasterDetailCopyLink", null);
                    title = Language.Phrase("MasterDetailCopyLink", true);
                    url = GetCopyUrl(Config.TableShowDetail + "=tr_request_approval_history");
                    links += "<li><a class=\"dropdown-item ew-row-link ew-detail-copy\" data-action=\"add\" data-caption=\"" + title + "\" href=\"" + HtmlEncode(AppPath(url)) + "\">" + caption + "</a></li>";
                    if (!Empty(detailCopyTblVar))
                        detailCopyTblVar += ",";
        ...
        protected void RenderListOptionsExt() {
            ...
            if ((DetailPages?["tr_request_detail"]?.Visible ?? false) && Security.AllowList(CurrentProjectID + "tr_request_detail")) {
                ...
                url = "TrRequestDetailPreview?t=tr_request&f=" + Encrypt(sqlwrk + "|" + String.Join("|", masterKeys));
                btngrp = "<ul data-table=\"tr_request_detail\" data-url=\"" + AppPath(url) + "\" class=\"ew-detail-btn-group dropdown-menu\">";
                if (Security.AllowList(CurrentProjectID + "tr_request")) {
                    ...
                    detaillnk = AppPath("TrRequestDetailList?" + Config.TableShowMaster + "=tr_request&" + ForeignKeyUrl("fk_id", id.CurrentValue) + "");
                ...
                if (detailPage?.DetailEdit && Security.CanEdit && Security.AllowEdit(CurrentProjectID + "tr_request") ?? false) {
                    caption = Language.Phrase("MasterDetailEditLink", null);
                    title = Language.Phrase("MasterDetailEditLink", true);
                    url = AppPath(GetEditUrl(Config.TableShowDetail + "=tr_request_detail"));
                    btngrp += "<a class=\"dropdown-item\" href=\"#\" data-ew-action=\"redirect\" data-url=\"" + HtmlEncode(url) + "\">" + caption + "</a>";
                }
                if (detailPage?.DetailAdd && Security.CanAdd && Security.AllowAdd(CurrentProjectID + "tr_request") ?? false) {
                    caption = Language.Phrase("MasterDetailCopyLink", null);
                    title = Language.Phrase("MasterDetailCopyLink", true);
                    url = AppPath(GetCopyUrl(Config.TableShowDetail + "=tr_request_detail"));
                    btngrp += "<a class=\"dropdown-item\" href=\"#\" data-ew-action=\"redirect\" data-url=\"" + HtmlEncode(url) + "\">" + caption + "</a>";
                }
                btngrp += "</ul>";
                if (link != "") {
            ...
            if ((DetailPages?["tr_request_approver"]?.Visible ?? false) && Security.AllowList(CurrentProjectID + "tr_request_approver")) {
                ...
                url = "TrRequestApproverPreview?t=tr_request&f=" + Encrypt(sqlwrk + "|" + String.Join("|", masterKeys));
                ...
                if (Security.AllowList(CurrentProjectID + "tr_request")) {
                    ...
                    detaillnk = AppPath("TrRequestApproverList?" + Config.TableShowMaster + "=tr_request&" + ForeignKeyUrl("fk_id", id.CurrentValue) + "");
                ...
                if (detailPage?.DetailEdit && Security.CanEdit && Security.AllowEdit(CurrentProjectID + "tr_request") ?? false) {
                    caption = Language.Phrase("MasterDetailEditLink", null);
                    title = Language.Phrase("MasterDetailEditLink", true);
                    url = AppPath(GetEditUrl(Config.TableShowDetail + "=tr_request_approver"));
                    btngrp += "<a class=\"dropdown-item\" href=\"#\" data-ew-action=\"redirect\" data-url=\"" + HtmlEncode(url) + "\">" + caption + "</a>";
                }
                btngrp += "</ul>";
                if (link != "") {
            ...
            if ((DetailPages?["tr_request_approval_history"]?.Visible ?? false) && Security.AllowList(CurrentProjectID + "tr_request_approval_history")) {
                ...
                url = "TrRequestApprovalHistoryPreview?t=tr_request&f=" + Encrypt(sqlwrk + "|" + String.Join("|", masterKeys));
                ...
                if (Security.AllowList(CurrentProjectID + "tr_request")) {
                    ...
                    detaillnk = AppPath("TrRequestApprovalHistoryList?" + Config.TableShowMaster + "=tr_request&" + ForeignKeyUrl("fk_id", id.CurrentValue) + "");
                ...
                if (detailPage?.DetailEdit && Security.CanEdit && Security.AllowEdit(CurrentProjectID + "tr_request") ?? false) {
                    caption = Language.Phrase("MasterDetailEditLink", null);
                    title = Language.Phrase("MasterDetailEditLink", true);
                    url = AppPath(GetEditUrl(Config.TableShowDetail + "=tr_request_approval_history"));
                    btngrp += "<a class=\"dropdown-item\" href=\"#\" data-ew-action=\"redirect\" data-url=\"" + HtmlEncode(url) + "\">" + caption + "</a>";
                }
                btngrp += "</ul>";
                if (link != "") {
        ...
        protected void SetupOtherOptions() {
            ...
            if (ModalAdd && !IsMobile())
                item.Body = $@"<a class=""ew-add-edit ew-add"" title=""{addTitle}"" data-table=""tr_request"" data-caption=""{addTitle}"" data-ew-action=""modal"" data-action=""add"" data-ajax=""" + (UseAjaxActions ? "true" : "false") + "\" data-url=\"" + HtmlEncode(AppPath(AddUrl)) + "\" data-btn=\"AddBtn\">" + Language.Phrase("AddLink") + "</a>";
            else
            ...
            item = option.Add("detailadd_tr_request_detail");
            trRequestDetail = Resolve("tr_request_detail")!;
            if (trRequestDetail != null) {
                caption = Language.Phrase("Add") + "&nbsp;" + Caption + "/" + trRequestDetail.Caption;
                title = Language.Phrase("Add", true) + "&nbsp;" + Caption + "/" + trRequestDetail.Caption;
                item.Body = "<a class=\"ew-detail-add-group ew-detail-add\" title=\"" + title + "\" data-caption=\"" + title + "\" href=\"" + HtmlEncode(AppPath(GetAddUrl(Config.TableShowDetail + "=tr_request_detail"))) + "\">" + caption + "</a>";
                item.Visible = (trRequestDetail.DetailAdd && Security.AllowAdd(CurrentProjectID + "tr_request") && Security.CanAdd);
                if (item.Visible) {
                    if (detailTableLink != "")
                        detailTableLink += ",";
            ...
            item = option.Add("detailadd_tr_request_approver");
            trRequestApprover = Resolve("tr_request_approver")!;
            if (trRequestApprover != null) {
                caption = Language.Phrase("Add") + "&nbsp;" + Caption + "/" + trRequestApprover.Caption;
                title = Language.Phrase("Add", true) + "&nbsp;" + Caption + "/" + trRequestApprover.Caption;
                item.Body = "<a class=\"ew-detail-add-group ew-detail-add\" title=\"" + title + "\" data-caption=\"" + title + "\" href=\"" + HtmlEncode(AppPath(GetAddUrl(Config.TableShowDetail + "=tr_request_approver"))) + "\">" + caption + "</a>";
                item.Visible = (trRequestApprover.DetailAdd && Security.AllowAdd(CurrentProjectID + "tr_request") && Security.CanAdd);
                if (item.Visible) {
                    if (detailTableLink != "")
                        detailTableLink += ",";
            ...
            item = option.Add("detailadd_tr_request_approval_history");
            trRequestApprovalHistory = Resolve("tr_request_approval_history")!;
            if (trRequestApprovalHistory != null) {
                caption = Language.Phrase("Add") + "&nbsp;" + Caption + "/" + trRequestApprovalHistory.Caption;
                title = Language.Phrase("Add", true) + "&nbsp;" + Caption + "/" + trRequestApprovalHistory.Caption;
                item.Body = "<a class=\"ew-detail-add-group ew-detail-add\" title=\"" + title + "\" data-caption=\"" + title + "\" href=\"" + HtmlEncode(AppPath(GetAddUrl(Config.TableShowDetail + "=tr_request_approval_history"))) + "\">" + caption + "</a>";
                item.Visible = (trRequestApprovalHistory.DetailAdd && Security.AllowAdd(CurrentProjectID + "tr_request") && Security.CanAdd);
                if (item.Visible) {
                    if (detailTableLink != "")
                        detailTableLink += ",";
            ...
            item.Body = $@"<button type=""button"" class=""ew-action ew-multi-delete"" title=""{deleteTitle}"" data-caption=""{deleteTitle}"" form=""ftr_requestlist""" +
            ...
            item.Body = $@"<button type=""button"" class=""ew-action ew-multi-update"" title=""{updateTitle}"" data-table=""tr_request"" data-caption=""{updateTitle}"" form=""ftr_requestlist"" data-ew-action=""" +
            ...
            if (UseColumnVisibility) {
                ...
                CreateColumnOption(option.Add("Request_Currency")); // DN
            }
            ...
            item.Body = "<a class=\"ew-save-filter\" data-form=\"ftr_requestsrch\" data-ew-action=\"none\">" + Language.Phrase("SaveCurrentFilter") + "</a>";
            item.Visible = true;
            item = FilterOptions.Add("deletefilter");
            item.Body = "<a class=\"ew-delete-filter\" data-form=\"ftr_requestsrch\" data-ew-action=\"none\">" + Language.Phrase("DeleteFilter") + "</a>";
            item.Visible = true;
        ...
        public void RenderOtherOptions()
        {
            ...
            var options = OtherOptions;
                ...
                foreach (var (key, act) in ListActions.Items.Where(kvp => kvp.Value.Select == Config.ActionMultiple)) {
                    ...
                    item.Body = "<<button type=\"button\" class=\"btn btn-default ew-action ew-list-action\" title=\"" + HtmlEncode(caption) + "\" data-caption=\"" + HtmlEncode(caption) + "\" data-ew-action=\"submit\" form=\"ftr_requestlist\"" + act.ToDataAttrs() + ">" + icon + "</button>";
        ...
        public async Task SetupRow()
        {
            ...
            RowAttrs.Add("data-rowindex", ConvertToString(trRequestList.RowCount));
            RowAttrs.Add("data-key", GetKey(true));
            RowAttrs.Add("id", "r" + ConvertToString(trRequestList.RowCount) + "_tr_request");
            RowAttrs.Add("data-rowtype", ConvertToString((int)RowType));
            RowAttrs.AppendClass(trRequestList.RowCount % 2 != 1 ? "ew-table-alt-row" : "");
            if (IsAdd && trRequestList.RowType == RowType.Add || IsEdit && trRequestList.RowType == RowType.Edit) // Inline-Add/Edit row
        ...
        protected void LoadSearchValues() {
            ...
            if (Query.ContainsKey("v_id"))
                id.AdvancedSearch.SearchCondition = Get("v_id");
            if (Query.ContainsKey("y_id"))
                id.AdvancedSearch.SearchValue2 = Get("y_id");
            if (!Empty(id.AdvancedSearch.SearchValue2) && Command == "")
                Command = "search";
            if (Query.ContainsKey("w_id"))
                id.AdvancedSearch.SearchOperator2 = Get("w_id");
            ...
            // Request_Currency
            if (!IsAddOrEdit)
                if (Query.ContainsKey("x_Request_Currency"))
                    Request_Currency.AdvancedSearch.SearchValue = Get("x_Request_Currency");
                else
                    Request_Currency.AdvancedSearch.SearchValue = Get("Request_Currency"); // Default Value // DN
            if (!Empty(Request_Currency.AdvancedSearch.SearchValue) && Command == "")
                Command = "search";
            if (Query.ContainsKey("z_Request_Currency"))
                Request_Currency.AdvancedSearch.SearchOperator = Get("z_Request_Currency");
        }
        ...
        public async Task RenderRow()
        {
            ...
            if (RowType == RowType.View) {
                ...
                if (!IsExport())
                    Request_Currency.ViewValue = Request_Currency.HighlightValue(Request_Currency.ViewValue);
            } else if (RowType == RowType.Search) {
                // Request_No
                if (Request_No.UseFilter && !Empty(Request_No.AdvancedSearch.SearchValue)) {
                    Request_No.EditValue = ConvertToString(Request_No.AdvancedSearch.SearchValue).Split(Config.MultipleOptionSeparator).ToList();
                }

                // Reference_Doc
                if (Reference_Doc.UseFilter && !Empty(Reference_Doc.AdvancedSearch.SearchValue)) {
                    Reference_Doc.EditValue = ConvertToString(Reference_Doc.AdvancedSearch.SearchValue).Split(Config.MultipleOptionSeparator).ToList();
                }

                // Reason
                if (Reason.UseFilter && !Empty(Reason.AdvancedSearch.SearchValue)) {
                    Reason.EditValue = ConvertToString(Reason.AdvancedSearch.SearchValue).Split(Config.MultipleOptionSeparator).ToList();
                }

                // Request_By
                if (Request_By.UseFilter && !Empty(Request_By.AdvancedSearch.SearchValue)) {
                    Request_By.EditValue = ConvertToString(Request_By.AdvancedSearch.SearchValue).Split(Config.MultipleOptionSeparator).ToList();
                }

                // Request_By_Name
                if (Request_By_Name.UseFilter && !Empty(Request_By_Name.AdvancedSearch.SearchValue)) {
                    Request_By_Name.EditValue = ConvertToString(Request_By_Name.AdvancedSearch.SearchValue).Split(Config.MultipleOptionSeparator).ToList();
                }

                // Request_By_Position
                if (Request_By_Position.UseFilter && !Empty(Request_By_Position.AdvancedSearch.SearchValue)) {
                    Request_By_Position.EditValue = ConvertToString(Request_By_Position.AdvancedSearch.SearchValue).Split(Config.MultipleOptionSeparator).ToList();
                }

                // Request_By_Branch
                if (Request_By_Branch.UseFilter && !Empty(Request_By_Branch.AdvancedSearch.SearchValue)) {
                    Request_By_Branch.EditValue = ConvertToString(Request_By_Branch.AdvancedSearch.SearchValue).Split(Config.MultipleOptionSeparator).ToList();
                }

                // Request_By_Region
                if (Request_By_Region.UseFilter && !Empty(Request_By_Region.AdvancedSearch.SearchValue)) {
                    Request_By_Region.EditValue = ConvertToString(Request_By_Region.AdvancedSearch.SearchValue).Split(Config.MultipleOptionSeparator).ToList();
                }

                // Request_Industry
                if (Request_Industry.UseFilter && !Empty(Request_Industry.AdvancedSearch.SearchValue)) {
                    Request_Industry.EditValue = ConvertToString(Request_Industry.AdvancedSearch.SearchValue).Split(Config.MultipleOptionSeparator).ToList();
                }

                // Customer_ID
                if (Customer_ID.UseFilter && !Empty(Customer_ID.AdvancedSearch.SearchValue)) {
                    Customer_ID.EditValue = ConvertToString(Customer_ID.AdvancedSearch.SearchValue).Split(Config.MultipleOptionSeparator).ToList();
                }

                // Customer_Name
                if (Customer_Name.UseFilter && !Empty(Customer_Name.AdvancedSearch.SearchValue)) {
                    Customer_Name.EditValue = ConvertToString(Customer_Name.AdvancedSearch.SearchValue).Split(Config.MultipleOptionSeparator).ToList();
                }

                // SAP_Total
                if (SAP_Total.UseFilter && !Empty(SAP_Total.AdvancedSearch.SearchValue)) {
                    SAP_Total.EditValue = ConvertToString(SAP_Total.AdvancedSearch.SearchValue).Split(Config.MultipleOptionSeparator).ToList();
                }

                // Override_Total
                if (Override_Total.UseFilter && !Empty(Override_Total.AdvancedSearch.SearchValue)) {
                    Override_Total.EditValue = ConvertToString(Override_Total.AdvancedSearch.SearchValue).Split(Config.MultipleOptionSeparator).ToList();
                }

                // Variance_Total
                if (Variance_Total.UseFilter && !Empty(Variance_Total.AdvancedSearch.SearchValue)) {
                    Variance_Total.EditValue = ConvertToString(Variance_Total.AdvancedSearch.SearchValue).Split(Config.MultipleOptionSeparator).ToList();
                }

                // Variance_Percent
                if (Variance_Percent.UseFilter && !Empty(Variance_Percent.AdvancedSearch.SearchValue)) {
                    Variance_Percent.EditValue = ConvertToString(Variance_Percent.AdvancedSearch.SearchValue).Split(Config.MultipleOptionSeparator).ToList();
                }
                ...
                // Next_Approver
                if (Next_Approver.UseFilter && !Empty(Next_Approver.AdvancedSearch.SearchValue)) {
                    Next_Approver.EditValue = ConvertToString(Next_Approver.AdvancedSearch.SearchValue).Split(Config.MultipleOptionSeparator).ToList();
                }

                // Request_PIC
                if (Request_PIC.UseFilter && !Empty(Request_PIC.AdvancedSearch.SearchValue)) {
                    Request_PIC.EditValue = ConvertToString(Request_PIC.AdvancedSearch.SearchValue).Split(Config.MultipleOptionSeparator).ToList();
                }

                // Request_Status
                if (Request_Status.UseFilter && !Empty(Request_Status.AdvancedSearch.SearchValue)) {
                    Request_Status.EditValue = ConvertToString(Request_Status.AdvancedSearch.SearchValue).Split(Config.MultipleOptionSeparator).ToList();
                }

                // List_Approver
                if (List_Approver.UseFilter && !Empty(List_Approver.AdvancedSearch.SearchValue)) {
                    List_Approver.EditValue = ConvertToString(List_Approver.AdvancedSearch.SearchValue).Split(Config.MultipleOptionSeparator).ToList();
                }
                ...
                if (!Request_Currency.Raw)
                    Request_Currency.AdvancedSearch.SearchValue = HtmlDecode(Request_Currency.AdvancedSearch.SearchValue);
                Request_Currency.EditValue = HtmlEncode(Request_Currency.AdvancedSearch.SearchValue);
        ...
        protected void SetupSearchOptions() {
            ...
            item.Body = "<a class=\"btn btn-default ew-search-toggle" + searchToggleClass + "\" role=\"button\" title=\"" + Language.Phrase("SearchPanel") + "\" data-caption=\"" + Language.Phrase("SearchPanel") + "\" data-ew-action=\"search-toggle\" data-form=\"ftr_requestsrch\" aria-pressed=\"" + (searchToggleClass == " active" ? "true" : "false") + "\">" + Language.Phrase("SearchLink") + "</a>";
            item.Visible = true;
            ...
            if (ModalSearch && !IsMobile())
                item.Body = "<a class=\"btn btn-default ew-advanced-search\" title=\"" + Language.Phrase("AdvancedSearch", true) + "\" data-table=\"tr_request\" data-caption=\"" + Language.Phrase("AdvancedSearch", true) + "\" data-ew-action=\"modal\" data-url=\"" + AppPath("TrRequestSearch") + "\" data-btn=\"SearchBtn\">" + Language.Phrase("AdvancedSearch", false) + "</a>";
            else
                item.Body = "<a class=\"btn btn-default ew-advanced-search\" title=\"" + Language.Phrase("AdvancedSearch", true) + "\" data-caption=\"" + Language.Phrase("AdvancedSearch", true) + "\" href=\"" + AppPath("TrRequestSearch") + "\">" + Language.Phrase("AdvancedSearch", false) + "</a>";
            item.Visible = true;

            // Query builder button
            item = SearchOptions.Add("querybuilder");
            if (ModalSearch && !IsMobile())
                item.Body = "<a class=\"btn btn-default ew-query-builder\" title=\"" + Language.Phrase("QueryBuilder", true) + "\" data-table=\"tr_request\" data-caption=\"" + Language.Phrase("QueryBuilder", true) + "\" data-ew-action=\"modal\" data-url=\"" + AppPath("TrRequestQuery") + "\" data-btn=\"SearchBtn\">" + Language.Phrase("QueryBuilder", false) + "</a>";
            else
                item.Body = "<a class=\"btn btn-default ew-query-builder\" title=\"" + Language.Phrase("QueryBuilder", true) + "\" data-caption=\"" + Language.Phrase("QueryBuilder", true) + "\" href=\"" + AppPath("TrRequestQuery") + "\">" + Language.Phrase("QueryBuilder", false) + "</a>";
            item.Visible = true;

            // Search highlight button
            item = SearchOptions.Add("searchhighlight");
            item.Body = "<a class=\"btn btn-default ew-highlight active\" role=\"button\" title=\"" + Language.Phrase("Highlight") + "\" data-caption=\"" + Language.Phrase("Highlight") + "\" data-ew-action=\"highlight\" data-form=\"ftr_requestsrch\" data-name=\"" + HighlightName + "\">" + Language.Phrase("HighlightBtn") + "</a>";
