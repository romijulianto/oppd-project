...
public partial class OverridePS {
    ...
    /// logEmailList
    /// </summary>
    public static LogEmailList logEmailList
    {
        get => HttpData.Get<LogEmailList>("logEmailList")!;
        set => HttpData["logEmailList"] = value;
    }

    /// <summary>
    /// Page class for log_email
    /// </summary>
    public class LogEmailList : LogEmailListBase
    {
        // Constructor
        public LogEmailList(Controller controller) : base(controller)
        {
        }

        // Constructor
        public LogEmailList() : base()
        {
        }
    }
    ...
    /// Page base class
    /// </summary>
    public class LogEmailListBase : LogEmail
    {
        // Page ID
        ...
        // Page object name
        public string PageObjName = "logEmailList";

        // Title
        public string? Title = null; // Title for <title> tag

        // Grid form hidden field names
        public string FormName = "flog_emaillist";
        ...
        public LogEmailListBase()
        {
            ...
            MultiDeleteUrl = "LogEmailDelete";
            MultiUpdateUrl = "LogEmailUpdate";
        ...
        // Page name
        public string PageName => "LogEmailList";
        ...
        // Constructor
        public LogEmailListBase(Controller? controller = null): this() { // DN
            if (controller != null)
        ...
        public async Task<string> GetFilterList()
        {
            ...
            filters.Merge(JObject.Parse(Email_Type.AdvancedSearch.ToJson())); // Field Email_Type
            filters.Merge(JObject.Parse(Email_To.AdvancedSearch.ToJson())); // Field Email_To
            filters.Merge(JObject.Parse(Email_Cc.AdvancedSearch.ToJson())); // Field Email_Cc
            filters.Merge(JObject.Parse(Email_Subject.AdvancedSearch.ToJson())); // Field Email_Subject
            filters.Merge(JObject.Parse(Email_Body.AdvancedSearch.ToJson())); // Field Email_Body
        ...
        protected bool RestoreFilterList() {
            ...
            // Field Email_Type
            if (filter?.TryGetValue("x_Email_Type", out sv) ?? false) {
                Email_Type.AdvancedSearch.SearchValue = sv;
                Email_Type.AdvancedSearch.SearchOperator = filter["z_Email_Type"];
                Email_Type.AdvancedSearch.SearchCondition = filter["v_Email_Type"];
                Email_Type.AdvancedSearch.SearchValue2 = filter["y_Email_Type"];
                Email_Type.AdvancedSearch.SearchOperator2 = filter["w_Email_Type"];
                Email_Type.AdvancedSearch.Save();
            }

            // Field Email_To
            if (filter?.TryGetValue("x_Email_To", out sv) ?? false) {
                Email_To.AdvancedSearch.SearchValue = sv;
                Email_To.AdvancedSearch.SearchOperator = filter["z_Email_To"];
                Email_To.AdvancedSearch.SearchCondition = filter["v_Email_To"];
                Email_To.AdvancedSearch.SearchValue2 = filter["y_Email_To"];
                Email_To.AdvancedSearch.SearchOperator2 = filter["w_Email_To"];
                Email_To.AdvancedSearch.Save();
            }

            // Field Email_Cc
            if (filter?.TryGetValue("x_Email_Cc", out sv) ?? false) {
                Email_Cc.AdvancedSearch.SearchValue = sv;
                Email_Cc.AdvancedSearch.SearchOperator = filter["z_Email_Cc"];
                Email_Cc.AdvancedSearch.SearchCondition = filter["v_Email_Cc"];
                Email_Cc.AdvancedSearch.SearchValue2 = filter["y_Email_Cc"];
                Email_Cc.AdvancedSearch.SearchOperator2 = filter["w_Email_Cc"];
                Email_Cc.AdvancedSearch.Save();
            }

            // Field Email_Subject
            if (filter?.TryGetValue("x_Email_Subject", out sv) ?? false) {
                Email_Subject.AdvancedSearch.SearchValue = sv;
                Email_Subject.AdvancedSearch.SearchOperator = filter["z_Email_Subject"];
                Email_Subject.AdvancedSearch.SearchCondition = filter["v_Email_Subject"];
                Email_Subject.AdvancedSearch.SearchValue2 = filter["y_Email_Subject"];
                Email_Subject.AdvancedSearch.SearchOperator2 = filter["w_Email_Subject"];
                Email_Subject.AdvancedSearch.Save();
            }

            // Field Email_Body
            if (filter?.TryGetValue("x_Email_Body", out sv) ?? false) {
                Email_Body.AdvancedSearch.SearchValue = sv;
                Email_Body.AdvancedSearch.SearchOperator = filter["z_Email_Body"];
                Email_Body.AdvancedSearch.SearchCondition = filter["v_Email_Body"];
                Email_Body.AdvancedSearch.SearchValue2 = filter["y_Email_Body"];
                Email_Body.AdvancedSearch.SearchOperator2 = filter["w_Email_Body"];
                Email_Body.AdvancedSearch.Save();
            }
        ...
        public string BasicSearchWhere(bool def = false) {
            ...
            searchFlds.Add(Email_Type);
            searchFlds.Add(Email_To);
            searchFlds.Add(Email_Cc);
            searchFlds.Add(Email_Subject);
            searchFlds.Add(Email_Body);
        ...
        protected void SetupSortOrder() {
            ...
            if (Get("order", out StringValues sv)) {
                ...
                UpdateSort(id); // id
                UpdateSort(Email_Type); // Email_Type
                UpdateSort(Email_To); // Email_To
                UpdateSort(Email_Cc); // Email_Cc
                UpdateSort(Email_Subject); // Email_Subject
                UpdateSort(Email_Body); // Email_Body
        ...
        public async Task RenderListOptions()
        {
            ...
            if (isVisible) {
                if (ModalView && !IsMobile())
                    listOption?.SetBody($@"<a class=""ew-row-link ew-view"" title=""{viewcaption}"" data-table=""log_email"" data-caption=""{viewcaption}"" data-ew-action=""modal"" data-action=""view"" data-ajax=""" + (UseAjaxActions ? "true" : "false") + "\" data-url=\"" + HtmlEncode(AppPath(ViewUrl)) + "\" data-btn=\"null\">" + Language.Phrase("ViewLink") + "</a>");
                else
            ...
            if (isVisible) {
                if (ModalEdit && !IsMobile())
                    listOption?.SetBody($@"<a class=""ew-row-link ew-edit"" title=""{editcaption}"" data-table=""log_email"" data-caption=""{editcaption}"" data-ew-action=""modal"" data-action=""edit"" data-ajax=""" + (UseAjaxActions ? "true" : "false") + "\" data-url=\"" + HtmlEncode(AppPath(EditUrl)) + "\" data-btn=\"SaveBtn\">" + Language.Phrase("EditLink") + "</a>");
                else
            ...
            if (isVisible) {
                if (ModalAdd && !IsMobile())
                    listOption?.SetBody($@"<a class=""ew-row-link ew-copy"" title=""{copycaption}"" data-table=""log_email"" data-caption=""{copycaption}"" data-ew-action=""modal"" data-action=""add"" data-ajax=""" + (UseAjaxActions ? "true" : "false") + "\" data-url=\"" + HtmlEncode(AppPath(CopyUrl)) + "\" data-btn=\"AddBtn\">" + Language.Phrase("CopyLink") + "</a>");
                else
            ...
            if (listOption != null && !IsExport() && CurrentAction == "") {
                ...
                foreach (var (key, act) in ListActions.Items) {
                    if (act.Select == Config.ActionSingle && act.Allowed) {
                        ...
                        string link = "<li><button type=\"button\" class=\"dropdown-item ew-action ew-list-action\" data-caption=\"" + HtmlTitle(caption) + "\" data-ew-action=\"submit\" form=\"flog_emaillist\" data-key=\"" + KeyToJson(true) + "\"" + act.ToDataAttrs() + ">" + icon + " " + caption + "</button></li>";
                        if (!Empty(link)) {
                            links.Add(link);
                            if (Empty(body)) // Setup first button
                                body = "<button type=\"button\" class=\"btn btn-default ew-action ew-list-action\" title=\"" + HtmlTitle(caption) + "\" data-caption=\"" + HtmlTitle(caption) + "\" data-ew-action=\"submit\" form=\"flog_emaillist\" data-key=\"" + KeyToJson(true) + "\"" + act.ToDataAttrs() + ">" + icon + caption + "</button>";
                        }
                    }
                }
        ...
        protected void SetupOtherOptions() {
            ...
            if (ModalAdd && !IsMobile())
                item.Body = $@"<a class=""ew-add-edit ew-add"" title=""{addTitle}"" data-table=""log_email"" data-caption=""{addTitle}"" data-ew-action=""modal"" data-action=""add"" data-ajax=""" + (UseAjaxActions ? "true" : "false") + "\" data-url=\"" + HtmlEncode(AppPath(AddUrl)) + "\" data-btn=\"AddBtn\">" + Language.Phrase("AddLink") + "</a>";
            else
            ...
            item.Body = $@"<button type=""button"" class=""ew-action ew-multi-delete"" title=""{deleteTitle}"" data-caption=""{deleteTitle}"" form=""flog_emaillist""" +
            ...
            if (UseColumnVisibility) {
                ...
                CreateColumnOption(option.Add("Email_Type")); // DN
                CreateColumnOption(option.Add("Email_To")); // DN
                CreateColumnOption(option.Add("Email_Cc")); // DN
                CreateColumnOption(option.Add("Email_Subject")); // DN
                CreateColumnOption(option.Add("Email_Body")); // DN
            }
            ...
            item.Body = "<a class=\"ew-save-filter\" data-form=\"flog_emailsrch\" data-ew-action=\"none\">" + Language.Phrase("SaveCurrentFilter") + "</a>";
            item.Visible = true;
            item = FilterOptions.Add("deletefilter");
            item.Body = "<a class=\"ew-delete-filter\" data-form=\"flog_emailsrch\" data-ew-action=\"none\">" + Language.Phrase("DeleteFilter") + "</a>";
            item.Visible = true;
        ...
        public void RenderOtherOptions()
        {
            ...
            var options = OtherOptions;
                ...
                foreach (var (key, act) in ListActions.Items.Where(kvp => kvp.Value.Select == Config.ActionMultiple)) {
                    ...
                    item.Body = "<<button type=\"button\" class=\"btn btn-default ew-action ew-list-action\" title=\"" + HtmlEncode(caption) + "\" data-caption=\"" + HtmlEncode(caption) + "\" data-ew-action=\"submit\" form=\"flog_emaillist\"" + act.ToDataAttrs() + ">" + icon + "</button>";
        ...
        public async Task SetupRow()
        {
            ...
            RowAttrs.Add("data-rowindex", ConvertToString(logEmailList.RowCount));
            RowAttrs.Add("data-key", GetKey(true));
            RowAttrs.Add("id", "r" + ConvertToString(logEmailList.RowCount) + "_log_email");
            RowAttrs.Add("data-rowtype", ConvertToString((int)RowType));
            RowAttrs.AppendClass(logEmailList.RowCount % 2 != 1 ? "ew-table-alt-row" : "");
            if (IsAdd && logEmailList.RowType == RowType.Add || IsEdit && logEmailList.RowType == RowType.Edit) // Inline-Add/Edit row
        ...
        public async Task RenderRow()
        {
            ...
            if (RowType == RowType.View) {
                ...
                Email_Type.TooltipValue = "";

                // Email_To
                Email_To.HrefValue = "";
                Email_To.TooltipValue = "";

                // Email_Cc
                Email_Cc.HrefValue = "";
                Email_Cc.TooltipValue = "";

                // Email_Subject
                Email_Subject.HrefValue = "";
                Email_Subject.TooltipValue = "";

                // Email_Body
                Email_Body.HrefValue = "";
                Email_Body.TooltipValue = "";
            }
        ...
        protected void SetupSearchOptions() {
            ...
            item.Body = "<a class=\"btn btn-default ew-search-toggle" + searchToggleClass + "\" role=\"button\" title=\"" + Language.Phrase("SearchPanel") + "\" data-caption=\"" + Language.Phrase("SearchPanel") + "\" data-ew-action=\"search-toggle\" data-form=\"flog_emailsrch\" aria-pressed=\"" + (searchToggleClass == " active" ? "true" : "false") + "\">" + Language.Phrase("SearchLink") + "</a>";
            item.Visible = true;
